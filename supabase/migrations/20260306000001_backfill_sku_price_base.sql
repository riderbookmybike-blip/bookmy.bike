...
DO $$
BEGIN
  IF to_regclass('public.cat_items') IS NULL OR to_regclass('public.cat_price_state') IS NULL THEN
    RAISE NOTICE 'Skipping SKU price_base backfill: required tables missing';
    RETURN;
  END IF;
