-- Finalize business-confirmed dealer enrichment inputs (2026-03-20)
-- Inputs confirmed by business:
-- 1) automax-yamaha city -> Vasai final
-- 2) suryodaya legal + Vasai East/West branch anchors
-- 3) myscooty legal name -> Myscooty

BEGIN;

-- Automax Yamaha: clear pending ambiguity flags and set business-confirmed legal metadata.
...
-- Myscooty: business-confirmed legal name.
UPDATE public.id_tenants t
SET
  name = 'Myscooty',
  config = (
...
-- Suryodaya: keep legal name finalized and clear address-verification flag.
UPDATE public.id_tenants t
SET
...
-- Ensure primary SHOWROOM for Suryodaya is Vasai East.
...
-- Add Vasai West showroom branch for Suryodaya if missing.
...
SELECT
  t.id,
  'Suryodaya Bajaj - Vasai West Showroom',
  'SHOWROOM',
