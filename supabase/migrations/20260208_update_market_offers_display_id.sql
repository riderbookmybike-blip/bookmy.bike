-- Migration: 20260208_update_market_offers_display_id
-- Description: Use tenant display_id (studio ID) as dealer_name in market offers
...
BEGIN
    ...
    WITH relevant_dealers AS (
        ...
        SELECT t.id as tenant_id, COALESCE(t.display_id, t.name) as tenant_name, true as serviceable
    ...
    fallback_dealers AS (
        ...
        SELECT t.id as tenant_id, COALESCE(t.display_id, t.name) as tenant_name, false as serviceable
        FROM public.id_tenants t
        JOIN public.id_locations l ON l.tenant_id = t.id
