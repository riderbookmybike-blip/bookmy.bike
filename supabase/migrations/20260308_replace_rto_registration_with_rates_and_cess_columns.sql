...
BEGIN
  IF to_regclass('public.cat_price_mh') IS NOT NULL THEN
    ...
    -- Backfill amount mirrors from current cess columns if present.
    UPDATE public.cat_price_mh
    SET
      rto_cess_amount_state = COALESCE(rto_cess_amount_state, rto_state_roadtaxcess, rto_state_cess),
      rto_cess_amount_bh = COALESCE(rto_cess_amount_bh, rto_bh_roadtaxcess, rto_bh_cess),
      rto_cess_amount_company = COALESCE(rto_cess_amount_company, rto_company_roadtaxcess, rto_company_cess);

    -- Keep ordered view compatible before dropping column.
    ...
    -- Remove deprecated consolidated static column as requested.
    ALTER TABLE public.cat_price_mh
