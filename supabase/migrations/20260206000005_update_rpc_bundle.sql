-- Migration: update_get_market_best_offers_v2
-- Description: Updates the RPC to include bundle_ids and bundle_value (sum of MRPs of bundled items)

DROP FUNCTION IF EXISTS public.get_market_best_offers(text, text);

ALTER TABLE public.id_dealer_pricing_rules
  ADD COLUMN IF NOT EXISTS inclusion_type TEXT,
  ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;

ALTER TABLE public.cat_items
  ADD COLUMN IF NOT EXISTS price_base NUMERIC DEFAULT 0;
...
BEGIN
    ...
    ranked_offers AS (
        ...
        -- Only consider Vehicle Rules for the main ranking (type SKU/color is handled by logic calling this)
        -- We assume 'r' here are VEHICLE rules because the caller filters output by vehicle_color_id.
        -- BUT id_dealer_pricing_rules mixes vehicles and accessories.
        -- We should strictly filter 'r' to be a Vehicle SKU rule? 
        -- The caller uses the result `vehicle_color_id` to match with their Catalog SKUs.
        -- If we return an Accessory Rule here, it won't match any Vehicle SKU ID, so it gets filtered out by UI.
        -- So it's safe.
    )
    SELECT 
        ro.vehicle_color_id,
