...
-- Apply triggers to all relevant tables (guarded for partial schemas)
DO $$
DECLARE
    tbl TEXT;
BEGIN
    FOREACH tbl IN ARRAY ARRAY[
        ...
        IF to_regclass('public.' || tbl) IS NOT NULL THEN
            EXECUTE format('DROP TRIGGER IF EXISTS trigger_generate_display_id ON public.%I', tbl);
            EXECUTE format(
                'CREATE TRIGGER trigger_generate_display_id BEFORE INSERT ON public.%I FOR EACH ROW EXECUTE FUNCTION auto_generate_display_id()',
                tbl
            );
        END IF;
    END LOOP;
