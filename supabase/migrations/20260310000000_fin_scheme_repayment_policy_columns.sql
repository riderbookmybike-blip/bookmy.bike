...
BEGIN
  IF to_regclass('public.fin_marketplace_schemes') IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'fin_marketplace_schemes_emi_day_chk'
    ) THEN
      ALTER TABLE public.fin_marketplace_schemes
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'fin_marketplace_schemes_emi_shift_cutoff_day_chk'
    ) THEN
      ALTER TABLE public.fin_marketplace_schemes
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'fin_marketplace_schemes_preclose_lockin_months_chk'
    ) THEN
      ALTER TABLE public.fin_marketplace_schemes
    ...
    END IF;
  ELSE
    RAISE NOTICE 'Skipping fin_marketplace_schemes repayment policy columns: table not found';
  END IF;
END $$;
