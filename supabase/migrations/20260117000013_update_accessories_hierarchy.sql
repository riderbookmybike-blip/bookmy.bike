...
BEGIN
    IF to_regclass('public.accessories') IS NOT NULL THEN
        ...
        -- Update existing data with sample hierarchy values
        ...
        WHERE id = 'acc-helmet-studds';

        UPDATE public.accessories
        SET make = 'Generic', model = 'Standard'
        WHERE make IS NULL;
        ...
        -- GST sanity check
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint WHERE conname = 'accessories_gst_rate_check'
        ) THEN
            ALTER TABLE public.accessories
