...
alter table if exists public.cat_skus_linear
...
begin
  if to_regclass('public.cat_skus_linear') is null then
    raise notice 'Skipping ex_showroom_mh backfill: public.cat_skus_linear not found';
    return;
  end if;
...
begin
  if to_regclass('public.cat_skus_linear') is null then
    raise notice 'Skipping ex_showroom_mh trigger setup: public.cat_skus_linear not found';
    return;
  end if;

  execute 'drop trigger if exists trg_sync_linear_ex_showroom_mh on public.cat_skus_linear';
  execute 'create trigger trg_sync_linear_ex_showroom_mh before insert or update on public.cat_skus_linear for each row execute function public.sync_linear_ex_showroom_mh()';
end
$$;
