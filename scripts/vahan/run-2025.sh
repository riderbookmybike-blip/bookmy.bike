...
LOG_FILE="tmp/vahan_2025_runner.log"
...
console.log(`Pending RTOs for ${year}: ${pending.join(', ') || 'NONE'}`);
process.stdout.write(String(pending.length));
NODE
}

echo "===== 2025 DATA SCRAPING START $(date) =====" | tee -a "$LOG_FILE"
echo "Target: All 58 MH RTOs, Year: $YEAR" | tee -a "$LOG_FILE"
...
for i in $(seq 1 "$MAX_BATCHES"); do
  ...
  if [ "$pending" = "0" ]; then
    echo "✅ All 58 MH RTOs scraped for year $YEAR." | tee -a "$LOG_FILE"
    break
  fi

  echo "⏳ Sleeping ${SLEEP_SECS}s before next batch..." | tee -a "$LOG_FILE"
  sleep "$SLEEP_SECS"
done

echo "===== 2025 SCRAPING COMPLETE $(date) =====" | tee -a "$LOG_FILE"
