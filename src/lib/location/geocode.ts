...
interface GeocodeResult {
    ...
    source: 'CACHE' | 'API' | 'MOCK' | 'ERROR' | 'OSM';
}
 ...
 * 4. Fallback to OpenStreetMap (Free)
 * 5. If all fail, use Mock/Fallback (Free)
 */
export async function getSmartPincode(lat: number, lng: number, googleMapsKey?: string): Promise<GeocodeResult> {
    // 1. Check Cache
    ...
    // 2. Try Google Maps API if Key Provided
    if (googleMapsKey) {
        try {
            ...
            if (data.status === 'OK' && data.results?.[0]) {
                ...
                if (!taluka)
                    taluka = addressComponents.find((c: any) =>
            ...
            console.error('Google Maps API failed, falling back to OSM', error);
        }
    }

    // 3. Fallback to OpenStreetMap (Nominatim)
    try {
        console.warn('Using OpenStreetMap Fallback');
        const res = await fetch(
            `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addressdetails=1`,
            {
                headers: {
                    'User-Agent': 'BookMyBike/1.0 (web-app)',
                },
            }
        );
        const data = await res.json();
        ...
        if (data && data.address) {
            ...
            const taluka = data.address.suburb || data.address.city || data.address.town || data.address.county;
