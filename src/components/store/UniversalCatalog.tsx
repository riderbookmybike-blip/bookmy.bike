...
export const UniversalCatalog = ({
    ...
    // Broadcast serviceability state so ShopperBottomNav can update its CTA
    useEffect(() => {
        window.dispatchEvent(
            new CustomEvent('catalogServiceabilityChanged', {
                detail: {
                    status: serviceability.status,
                    location: serviceability.location || null,
                },
            })
        );
    }, [serviceability.status, serviceability.location]);

    // Listen for ShopperBottomNav CTA click → open LocationPicker
    useEffect(() => {
        const handler = () => {
            setLocationPickerInitialPin(serviceability.pincode || '');
            setIsLocationPickerOpen(true);
        };
        window.addEventListener('openCatalogLocationPicker', handler);
        return () => window.removeEventListener('openCatalogLocationPicker', handler);
