...
export default function CascadingAccessorySelector({
    category,
    items,
    selectedId,
    onSelect,
    label,
}: CascadingAccessorySelectorProps) {
    ...
    const availableModels = useMemo(() => {
        ...
        return Array.from(
            new Set(items.filter(i => (i.make || 'Generic') === selectedMake).map(i => i.model || 'Standard'))
        );
    }, [items, selectedMake]);
            ...
            <div className="flex items-center justify-between">
                ...
                {activeItem && (
                    <div className="text-right">
                        ...
                        <p className="text-lg font-black italic font-mono text-brand-primary">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {availableVariants.map(variant => (
                            ...
                            >
                                <div>
                                    <p
                                        className={`text-[10px] font-black uppercase tracking-widest mb-1 ${variant.id === selectedId ? 'text-black' : 'text-slate-500'}`}
                                    >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {variant.size && (
                                            <span
                                                className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${variant.id === selectedId ? 'border-black/30 text-black' : 'border-slate-300 text-slate-400'}`}
                                            >
                                                {variant.size}
                                            </span>
                                        )}
                                ...
                                <div
                                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${variant.id === selectedId ? 'border-black bg-black text-brand-primary' : 'border-slate-300 text-transparent group-hover:border-brand-primary'}`}
                                >
