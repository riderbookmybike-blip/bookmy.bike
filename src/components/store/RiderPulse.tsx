...
export const RiderPulse = () => {
    ...
    return (
        <section className="py-24 md:py-32 bg-white dark:bg-[#020617] text-slate-900 dark:text-white overflow-hidden relative border-t border-slate-100 dark:border-white/5">
            <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-20 space-y-16 md:space-y-24 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {reviews.map((m, i) => (
                        <div
                            key={i}
                            className="group p-8 md:p-10 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-[3rem] space-y-8 hover:bg-white dark:hover:bg-white/[0.05] transition-all duration-500 relative hover:shadow-2xl hover:shadow-slate-200/50 dark:hover:shadow-none"
                        >
                            ...
                            <div className="pt-8 border-t border-slate-200 dark:border-white/5 flex items-center gap-4">
                                <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-200 dark:border-white/10">
                ...
                <div className="flex justify-center gap-3">
                    {[1, 2, 3, 4, 5, 6].map(dot => (
                        <div
                            key={dot}
                            className={`w-2 h-2 rounded-full ${dot === 1 ? 'bg-brand-primary w-6' : 'bg-slate-200 dark:bg-slate-800'} transition-all duration-500`}
                        />
                    ))}
                </div>
            </div>
