...
import { ShieldCheck, Zap, ShieldCheck as ShieldIcon, CheckCircle2, Package, ClipboardList, Wrench, Gift } from 'lucide-react';
...
import { productColors, mandatoryAccessories, optionalAccessories, mandatoryInsurance, insuranceAddons, serviceOptions, offerOptions, mandatoryInsurance as mandatoryIns } from '@/hooks/usePDPData';
...
export function PDPDesktop({ product, variantParam, data, handlers }: PDPDesktopProps) {
    ...
    const {
        handleColorChange, handleShareQuote, handleSaveQuote, handleBookingRequest,
    ...
    const totalMRP = (product.mrp || (baseExShowroom + 5000)) + rtoEstimates + (mandatoryIns.reduce((sum, i) => sum + i.price, 0)) + roadTax + accessoriesPrice + servicesPrice;
            ...
            >
                <div className="flex-1 flex items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4 min-w-[200px]">
                        ...
                        <div>
                            <p className={`text-xs md:text-sm font-black uppercase italic tracking-wider transition-colors ${isSelected ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}>{item.name}</p>
                            {item.description && <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 line-clamp-1">{item.description}</p>}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-end gap-8 pr-2">
                        <div className="text-center min-w-[60px] hidden md:block">
                            <span className="block text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5 opacity-50">Base</span>
                            <span className="text-[10px] font-bold font-mono text-slate-400">₹{(item.price || 0).toLocaleString()}</span>
                        </div>
                        {item.discountPrice > 0 ? (
                            <div className="text-center min-w-[60px] hidden md:block">
                                <span className="block text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5 opacity-50">Discount</span>
                                <span className="text-[10px] font-bold font-mono text-emerald-500">-₹{(item.price - item.discountPrice).toLocaleString()}</span>
                            </div>
                        ) : item.isOffer ? (
                            <div className="text-center min-w-[60px] hidden md:block">
                                <span className="block text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5 opacity-50">Offer</span>
                                <span className="text-[10px] font-bold font-mono text-emerald-500">-₹{(item.price || 0).toLocaleString()}</span>
                            </div>
                        ) : null}
                        <div className="text-center min-w-[30px]">
                            <span className="block text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5 opacity-50">Qty</span>
    ...
    const renderTabContent = () => {
        ...
        const TabHeader = ({ icon: Icon, title, subtext }: any) => (
            <div className="flex items-center gap-6 px-4 mb-8">
                ...
                <div>
                    <h3 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900 dark:text-white leading-none">{title}</h3>
        ...
        switch (configTab) {
                    ...
                    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="space-y-4">
                            {[60, 48, 36, 24].map(tenure => {
                                const emiValue = Math.round((loanAmount * (annualInterest / 12) * Math.pow(1 + (annualInterest / 12), tenure)) / (Math.pow(1 + (annualInterest / 12), tenure) - 1));
                                return (
                                    <ConfigItemRow
                                        key={tenure}
                                        item={{
            ...
            case 'REGISTRATION':
                const regItems = [
                    { id: 'STATE', name: 'State Registration', price: Math.round(baseExShowroom * 0.12), description: 'Standard RTO charges for your state.' },
                    { id: 'BH', name: 'Bharat Series (BH)', price: Math.round(baseExShowroom * 0.08), description: 'For frequent interstate travel.' },
                    { id: 'COMPANY', name: 'Company Registration', price: Math.round(baseExShowroom * 0.20), description: 'Corporate entity registration.' },
                ];
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="space-y-4 mb-10">
                            ...
                            {mandatoryInsurance.map(i => <ConfigItemRow key={i.id} item={i} isSelected={true} isMandatory />)}
                        </div>

                        <div className="space-y-4">
                            <SectionLabel text="Extra Coverage (Add-ons)" />
                            {insuranceAddons.map(i => <ConfigItemRow key={i.id} item={i} isSelected={selectedInsuranceAddons.includes(i.id)} onToggle={() => toggleInsuranceAddon(i.id)} />)}
                        </div>
                    </div>
                );
            case 'SERVICES':
                ...
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="space-y-4">
                            ...
                            {paidServices.map(srv => <ConfigItemRow key={srv.id} item={srv} isSelected={selectedServices.includes(srv.id)} onToggle={() => toggleService(srv.id)} />)}
                        </div>
                    </div>
                );
            case 'OFFERS':
                ...
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="space-y-4 mb-10">
                            ...
                            {standardWarranty.map((off: any) => <ConfigItemRow key={off.id} item={{ ...off, isOffer: true }} isSelected={selectedOffers.includes(off.id)} onToggle={() => toggleOffer(off.id)} />)}
                        </div>

                        <div className="space-y-4">
                            <SectionLabel text="Optional Extensions" />
                            {extendedWarranty.map((off: any) => <ConfigItemRow key={off.id} item={{ ...off, isOffer: true }} isSelected={selectedOffers.includes(off.id)} onToggle={() => toggleOffer(off.id)} />)}
                        </div>
                    </div>
                );
            default: return null;
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-white pb-32 transition-colors duration-1000 relative overflow-hidden">
            ...
            <div className="max-w-[1550px] mx-auto px-6 md:px-12 py-4 space-y-6 relative z-10">
                ...
                <DynamicHeader
                    breadcrumb={<>STORE / <span className="text-slate-500">{product.make}</span> / {product.model}</>}
                />
                        ...
                        <div className="bg-white dark:bg-slate-900/20 border border-slate-200 dark:border-white/5 rounded-[4rem] p-12 space-y-12 animate-in fade-in slide-in-from-bottom-12 duration-700 delay-100 shadow-2xl">
                            <TabNavigation
                                ...
                                tabs={[
                                    ...
                                    { id: 'ACCESSORIES', label: 'Accessories', count: selectedAccessories.length > 0 ? selectedAccessories.length : undefined },
                                    { id: 'INSURANCE', label: 'Insurance' },
                                    { id: 'REGISTRATION', label: 'Registration' },
                                    { id: 'SERVICES', label: 'Services', count: selectedServices.length > 0 ? selectedServices.length : undefined },
                                    { id: 'WARRANTY', label: 'Warranty', count: selectedOffers.length > 0 ? selectedOffers.length : undefined }
                                ]}
                            />
