...
import { getStableReviewCount, getVehicleUSP } from '@/utils/vehicleUtils';
...
const StarRating = ({ rating = 4.5, size = 10 }: { rating?: number; size?: number }) => {
    ...
    return (
        <div className="flex items-center gap-0.5">
            ...
            {hasHalfStar && (
                <StarHalf size={size} className="fill-[#F4B000] text-[#F4B000]" />
            )}
        </div>
    );
};

const ProductCard = ({ v, viewMode, downpayment, tenure }: {
    v: any;
    viewMode: 'grid' | 'list';
    downpayment: number;
    tenure: number;
    ...
    const usp = getVehicleUSP(v.make, v.model);
                ...
                <div className="flex-1 p-10 flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center py-6 border-y border-slate-100 dark:border-white/10 relative z-10 gap-8 mt-4">
                        <div className="space-y-1.5 w-1/4">
                            <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Engine</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{Math.round(v.displacement || 0)}{v.powerUnit || 'CC'}</p>
                        </div>
                        <div className="space-y-1.5 w-1/4">
                            <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Brakes</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{v.specifications?.brakes?.front || 'Drum'}</p>
                        </div>
                        <div className="space-y-1.5 w-1/4">
                            <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Type</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{v.bodyType || 'COMMUTER'}</p>
                        </div>
                        <div className="space-y-1.5 w-1/4 text-right">
                            <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Transmission</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{v.specifications?.transmission?.type || 'Manual'}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-6 relative z-10">
                        <div className="flex gap-16">
                            <div className="space-y-1">
                                <p className="text-[9px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">On-Road price</p>
                            ...
                            <div className="space-y-1">
                                <p className="text-[9px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">EMI / {tenure}mo</p>
    ...
    return (
        <div
            key={v.id}
            className="group bg-white dark:bg-slate-900/40 dark:backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col shadow-sm hover:shadow-2xl dark:hover:shadow-brand-primary/5 transition-all duration-500 dark:hover:border-white/20 min-h-[580px]"
        >
            <div className="h-[300px] bg-slate-50 dark:bg-white/[0.03] flex items-center justify-center relative p-4 border-b border-slate-100 dark:border-white/5 overflow-hidden group/card">
                ...
                <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
                    ...
                    {v.price?.discount && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/90 backdrop-blur-md text-white rounded-full shadow-lg shadow-emerald-500/20 animate-in fade-in slide-in-from-left-4 duration-500">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-[0.1em]">SAVE ₹{v.price.discount.toLocaleString('en-IN')}</span>
                        </div>
                    )}
                    {v.price?.surge && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/90 backdrop-blur-md text-white rounded-full shadow-lg shadow-amber-500/20 animate-in fade-in slide-in-from-left-4 duration-500">
                            <Zap size={10} className="fill-white" />
                            <span className="text-[9px] font-black uppercase tracking-[0.1em]">SURGE ₹{v.price.surge.toLocaleString('en-IN')}</span>
                        </div>
                    )}
                </div>
                <button
            ...
            <div className="p-8 space-y-8 flex-1 flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="flex items-center justify-between border-y border-slate-100 dark:border-white/10 py-5 bg-slate-50/50 dark:bg-white/[0.04] -mx-6 px-6 relative z-10">
                    <div className="space-y-1">
                        <p className="text-[8px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest leading-none">Price</p>
                        <span className="text-lg font-black text-slate-900 dark:text-white block tracking-tight">
                            ₹{basePrice.toLocaleString('en-IN')}
                        </span>
                    </div>
                    <div className="text-right space-y-1">
                        <p className="text-[8px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest leading-none">EMI/mo</p>
...
export function CatalogDesktop({ filters }: CatalogDesktopProps) {
    ...
    const activeFilterCount = useMemo(() => {
        let count = 0;
        if (filters.selectedMakes.length < brands.length) count++;
        if (filters.selectedCC.length > 0) count++;
        if (filters.selectedBrakes.length > 0) count++;
        if (filters.selectedWheels.length > 0) count++;
        if (filters.selectedFinishes.length > 0) count++;
        if (filters.selectedSeatHeight.length > 0) count++;
        if (filters.maxPrice < 1000000) count++;
        if (filters.maxEMI < 20000) count++;
        return count;
    }, [filters, brands]);
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-12 pt-28 pb-20 space-y-8 bg-white dark:bg-[#020617] transition-colors duration-500">
            ...
            <div className="flex flex-col md:flex-row items-end justify-between gap-6 pb-2">
                <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 dark:text-slate-400 leading-none">Catalog</p>
            ...
            <div className={`flex gap-8 xl:gap-20 transition-all duration-700 ${viewMode === 'grid' ? 'max-w-full' : ''}`}>
                                    ...
                                    <div className="space-y-8 p-6 bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl animate-in fade-in slide-in-from-top-2">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-4 gap-2">
                                                {(() => {
                                                    const allTenures = [3, 6, 9, 12, 18, 24, 30, 36, 42, 48, 54, 60];
                                                    const idx = allTenures.indexOf(tenure);
                                                    let start = Math.max(0, Math.min(idx - 2, allTenures.length - 4));
                                        ...
                                        <div className="space-y-6 pt-6 border-t border-slate-200/50 dark:border-white/5">
                                            <div className="space-y-4">
                                                <div className="flex justify-between items-end">
                                                    ...
                                                    <span className="text-sm font-black text-slate-900 dark:text-white italic">
                                                        {maxPrice >= 1000000 ? 'Any' : `Under ${(maxPrice / 100000).toFixed(1)}L`}
                                                    </span>
                                                </div>
                                                <div className="relative flex items-center h-6">
                                                    <input
                                            ...
                                            <div className="space-y-4">
                                                <div className="flex justify-between items-end">
                                                    ...
                                                    <span className="text-sm font-black text-brand-primary italic">
                                                        {maxEMI >= 20000 ? 'Any' : `Under ₹${maxEMI.toLocaleString('en-IN')}`}
                                                    </span>
                                                </div>
                                                <div className="relative flex items-center h-6">
                                                    <input
                ...
                <div className="flex-1 space-y-8">
                    ...
                    {/* Active Filter Chips */}
                    {(searchQuery || selectedCC.length > 0 || selectedBrakes.length > 0 || selectedWheels.length > 0 || selectedFinishes.length > 0 || selectedSeatHeight.length > 0) && (
                        <div className="flex flex-wrap items-center gap-2">
                            {searchQuery && (
                                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    ...
                                    <button onClick={() => setSearchQuery('')} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            )}
                            {selectedCC.map((cc: string) => (
                                <div key={cc} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    ...
                                    <button onClick={() => toggleFilter(setSelectedCC, cc)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            ))}
                            {selectedFinishes.map((finish: string) => (
                                <div key={finish} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    <span className="text-[9px] font-black uppercase text-slate-400">Finish</span>
                                    <span className="text-[10px] font-bold text-slate-900 dark:text-white">{finish}</span>
                                    <button onClick={() => toggleFilter(setSelectedFinishes, finish)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            ))}
                            {selectedSeatHeight.map((sh: string) => (
                                <div key={sh} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    <span className="text-[9px] font-black uppercase text-slate-400">Seat</span>
                                    <span className="text-[10px] font-bold text-slate-900 dark:text-white">{sh}</span>
                                    <button onClick={() => toggleFilter(setSelectedSeatHeight, sh)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            ))}
                            {selectedBrakes.map((brake: string) => (
                                <div key={brake} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    <span className="text-[9px] font-black uppercase text-slate-400">Brakes</span>
                                    <span className="text-[10px] font-bold text-slate-900 dark:text-white">{brake}</span>
                                    <button onClick={() => toggleFilter(setSelectedBrakes, brake)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            ))}
                            {selectedWheels.map((wheel: string) => (
                                <div key={wheel} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full">
                                    <span className="text-[9px] font-black uppercase text-slate-400">Wheels</span>
                                    <span className="text-[10px] font-bold text-slate-900 dark:text-white">{wheel}</span>
                                    <button onClick={() => toggleFilter(setSelectedWheels, wheel)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                        <X size={10} />
                                    </button>
                                </div>
                            ))}
                    ...
                    {/* Skeleton Loaders */}
                    {filters.isLoading && Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className="animate-pulse bg-slate-50 dark:bg-slate-800/50 rounded-3xl h-[400px]" />
                    ))}

                    <div
                        className={`grid gap-12 xl:gap-16 transition-all duration-700 ${viewMode === 'grid'
                            ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'
                            : 'grid-cols-1'
                            }`}
                    >
                        {/* Results Grid */}
                        {!filters.isLoading && results.map((v: any) => (
                            <ProductCard
                                key={v.id}
                                v={v}
                                viewMode={viewMode}
                    ...
                    <div className="fixed inset-0 z-[100] flex flex-col animate-in fade-in duration-300">
                        ...
                        <div className="relative flex-1 flex flex-col self-end w-full max-w-4xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden border-l border-slate-200 dark:border-white/10">
                            ...
                            <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-white/5">
                                <div className="flex items-center gap-4">
                                    <h3 className="text-xl font-black uppercase tracking-widest text-slate-900 dark:text-white">Filters</h3>
                                    {activeFilterCount > 0 && (
                            ...
                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                    ...
                                    <div className="space-y-12">
                                        <div className="space-y-6">
                                            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Finance Settings</h4>
                                            <div className="p-8 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-[2.5rem]">
                                                <div className="space-y-8">
                                                    <div className="space-y-4">
                                                        <div className="flex justify-between items-end">
                                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Downpayment</span>
                                                            <span className="text-xl font-black text-[#F4B000]">₹{downpayment.toLocaleString('en-IN')}</span>
                                                        </div>
                                                        <input
                                                            type="range"
                                                            min="5000"
                                                    ...
                                                    <div className="space-y-4">
                                                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tenure (Months)</span>
                                        ...
                                        <div className="space-y-6">
                                            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Search</h4>
                                    ...
                                    <div className="space-y-12">
                                        ...
                                        <FilterGroup
                                            title="Brake System"
                                            options={['Drum', 'Disc (Front)', 'Dual Disc', 'Single Channel ABS', 'Dual Channel ABS']}
