...
export default function QuoteForm({ isOpen, onClose, onSubmit, initialCustomerName, pincode }: QuoteFormProps) {
    ...
    return (
        <SlideOver isOpen={isOpen} onClose={onClose} title="Create New Quote" width="lg">
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
                        Only showing products enabled by Dealership
                    </p>
                    ...
                    {!selectedProduct ? (
                        ...
                        <div className="p-4 bg-blue-50 dark:bg-white/5 border border-blue-100 dark:border-white/10 rounded-lg">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h4 className="font-bold text-blue-900 dark:text-white">{selectedProduct.label}</h4>
                                    <p className="text-xs text-blue-700 dark:text-slate-400 mt-1 font-mono">
                                        {selectedProduct.sku}
                                    </p>
                                </div>
                                <button
                                    ...
                                    className="text-xs text-blue-600 dark:text-white hover:text-blue-800 dark:hover:text-slate-300 underline"
                                >
                                    Change Model
                                </button>
                            </div>
                            ...
                            {/* Color Selection */}
                            <div className="mt-6">
                                <label className="block text-[10px] font-black uppercase tracking-widest text-blue-800 dark:text-slate-400 mb-3">
                                    Choose Color
                                </label>
                                <div className="flex flex-wrap gap-3">
                                    {selectedProduct.availableColors?.map(color => (
                                        ...
                                        >
                                            <div
                                                className={`w-10 h-10 rounded-full border-2 transition-all ${selectedColor?.id === color.id ? 'border-blue-600 dark:border-white ring-4 ring-blue-600/10 dark:ring-white/20' : 'border-white/20'}`}
                                                style={{ backgroundColor: color.hexCode }}
                                            />
                                            <span className="text-[8px] font-bold text-blue-900 dark:text-white uppercase tracking-tighter text-center max-w-[50px] leading-tight">
                                                {color.name}
                                            </span>
                                        </button>
                                    ))}
                            ...
                            <div className="mt-6 pt-4 border-t border-blue-200 dark:border-white/10 flex justify-between items-end">
                                <span className="text-sm text-blue-800 dark:text-slate-400">Unit Price</span>
                ...
                <div className="pt-6 border-t border-slate-200 dark:border-white/10 flex justify-end gap-3">
                    ...
                    <button
                        ...
                        className={`px-6 py-2 text-white dark:text-black font-bold rounded-lg shadow-md transition-all ${
                            !selectedProduct || !customerName || !selectedColor
                                ? 'bg-slate-300 dark:bg-slate-800 dark:text-slate-600 cursor-not-allowed'
                                : 'bg-blue-600 hover:bg-blue-700 dark:bg-white dark:hover:bg-slate-200'
                        }`}
                    >
                        Generate Quote
                    </button>
