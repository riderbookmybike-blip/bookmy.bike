    ...
    const Row = (_index: number, item: Quote) => (
        <div className="pb-2 px-1">
            ...
            >
                ...
                <h3 className={`text-sm font-black tracking-tight mb-1 transition-colors ${item.id === selectedId ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'
                    }`}>
                    {item.customerName}
                </h3>
                ...
                <div className="flex items-center justify-between">
                    <div className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-tighter ${item.status === 'ACCEPTED' ? 'bg-emerald-500/10 text-emerald-500' :
                    ...
                    <span className={`text-[10px] font-bold ${item.id === selectedId ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-500'}`}>
    ...
    return (
        <div className="h-full flex flex-col bg-white dark:bg-slate-900/10 font-sans border-r border-slate-200 dark:border-white/5">
            ...
            <div className="p-6 pb-4 space-y-4 shrink-0">
                <div className="flex items-center justify-between">
                    <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Global Quotes</h2>
                    <button
                        onClick={onNewQuote}
