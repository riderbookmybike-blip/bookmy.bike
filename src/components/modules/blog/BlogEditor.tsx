...
export default function BlogEditor({ postId }: { postId?: string }) {
    ...
    if (loading) return <div className="p-20 text-center uppercase font-black tracking-widest animate-pulse">Loading Editor...</div>;
                    ...
                    <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-10 space-y-8 shadow-xl">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Read Time</label>
                                    <input
                                        type="text"
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between mb-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Article Content (Markdown)</label>
                                <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">MD Supported</span>
                            </div>
                            <textarea
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-8 space-y-6 shadow-lg">
                        <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Featured Image</h4>
                        </div>
                        <div className="aspect-video bg-slate-100 dark:bg-black/40 rounded-2xl overflow-hidden border border-dashed border-slate-300 dark:border-white/10 flex items-center justify-center relative group">
                            {data.image_url ? (
                                ...
                                <div className="text-center p-6">
                                    ...
                                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">No Image Selected</p>
                                </div>
                            )}
                        </div>
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Image URL</label>
                            <input
                                type="text"
                                placeholder="https://..."
                    ...
                    <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-8 space-y-8 shadow-lg">
                        <div className="space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Author</h4>
                            </div>
                            <input
                                type="text"
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Tags</h4>
                            </div>
                            <div className="flex flex-wrap gap-2 mb-3">
                                {data.tags.map(tag => (
                                    <span key={tag} className="flex items-center gap-1.5 px-3 py-1 bg-brand-primary/10 text-brand-primary rounded-full text-[8px] font-black uppercase tracking-widest">
                                        {tag}
                                        <X size={10} className="cursor-pointer" onClick={() => setData({ ...data, tags: data.tags.filter(t => t !== tag) })} />
                                    </span>
                                ))}
                            </div>
