...
export default function DocumentManager({ memberId, tenantId }: DocumentManagerProps) {
    ...
    const categories = [
        'PAN Card', 'Aadhar Card', 'Light Bill', 'Voting Card', 'Passport',
        'Electricity Bill', 'LIC Policy', 'Phone Bill',
    ...
    const handleStitchBatch = async () => {
        ...
        try {
            ...
            const blobs = await Promise.all(selectedDocs.map(async (doc) => {
            ...
            const loadedImages = await Promise.all(blobs.map(async (blob) => {
                const img = new Image();
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 font-sans">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Identity <span className="text-indigo-500">Vault</span></h3>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Immutable Document Protocol</p>
                </div>
            ...
            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-3"}>
                ...
                {isLoading ? (
                    Array(2).fill(0).map((_, i) => (
                        <div key={i} className="h-64 rounded-[2.5rem] bg-slate-100 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 animate-pulse" />
                    ))
                ) : (
                    documents.map((doc) => {
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 rounded-xl overflow-hidden bg-slate-100 dark:bg-black/20 flex items-center justify-center">
                                            {doc.file_type.includes('image') ? (
                                                <img src={getViewUrl(doc.path, doc.id)} alt={doc.purpose} className="w-full h-full object-cover" />
                                            ) : (
                                                <FileText size={24} className="text-slate-400" />
                                            )}
                                        </div>
                                        <div>
                                            <div className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1">{doc.purpose}</div>
                                            <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase truncate max-w-[200px]">{doc.metadata?.originalName || 'document.bin'}</h4>
                                        </div>
                                    </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                {doc.file_type.includes('image') ? (
                                    <img src={getViewUrl(doc.path, doc.id)} alt={doc.purpose} className="w-full h-full object-contain bg-slate-200 dark:bg-black/50 opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" />
                                ) : (
                                    <div className="w-full h-full flex flex-col items-center justify-center">
                                        <FileText size={64} className="text-slate-200 dark:text-slate-700 group-hover:text-indigo-500 transition-colors mb-4" />
                                        <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">PDF DOCUMENT</span>
                                    </div>
                                )}
                ...
                {!isSelectMode && (
                    <label className={`group relative h-64 border-2 border-dashed border-slate-200 dark:border-slate-700/50 hover:border-indigo-500/50 rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer transition-all bg-white/[0.02] hover:bg-indigo-500/[0.02] overflow-hidden ${viewMode === 'list' ? 'h-32' : ''}`}>
                        <input type="file" className="hidden" onChange={handleFileSelect} accept="image/*,application/pdf" multiple />
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Initialize Upload</span>
                        <span className="text-[8px] font-bold text-slate-400 dark:text-slate-600 mt-1 uppercase">Image/PDF [MAX 10MB]</span>
                    </label>
                )}
            </div>
                ...
                <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-8 duration-500">
                    <div className="bg-slate-900/90 backdrop-blur-xl border border-white/10 p-2 rounded-[2rem] shadow-2xl flex items-center gap-2">
                        <div className="px-6 py-2 border-r border-white/10">
                            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{selectedDocIds.length} Picked</span>
                        </div>
                        ...
                        <div className="relative group/ren">
                            <Button variant="ghost" className="h-14 px-6 rounded-2xl text-white hover:bg-white/5 transition-all flex items-center gap-3">
                                <Type size={18} />
                    ...
                    <div className="w-full max-w-6xl h-full flex flex-col items-center">
                        <div className="w-full flex items-center justify-between mb-8">
                            <div className="space-y-1">
                                <div className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em]">Archived Identity Asset</div>
                                <h4 className="text-3xl font-black text-white italic tracking-tighter uppercase italic">{viewingDoc.purpose}</h4>
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                {viewingDoc.file_type.includes('image') && (
                                    ...
                                    >
                                        <Scissors size={18} />
                                        <span className="text-[10px] font-black uppercase tracking-widest">Crop / Adjust</span>
                                    </Button>
                                )}
