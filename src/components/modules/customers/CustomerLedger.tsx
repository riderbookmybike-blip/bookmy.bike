    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="font-bold text-gray-700">Customer Ledger Account</h3>
                <span className="text-sm font-bold bg-gray-100 px-2 py-1 rounded">Closing Balance: ₹{balance.toLocaleString()}</span>
            </div>
                        ...
                        {entriesWithBalance.length === 0 ? (
                            ...
                            entriesWithBalance.map(e => (
                                <tr key={e.id} className="hover:bg-gray-50">
                                    <td className="px-4 py-2 font-mono text-gray-600">{e.transactionDate.split('T')[0]}</td>
