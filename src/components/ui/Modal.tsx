...
export default function Modal({ isOpen, onClose, title, children, size = 'md' }: ModalProps) {
    ...
    return (
        ...
        >
            ...
            <div
                className={`bg-white dark:bg-slate-900 rounded-[32px] w-full shadow-2xl relative z-10 flex flex-col max-h-[90vh] transition-all duration-200 transform ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'} ${sizeClasses[size]}`}
            >
                {/* Header */}
                <div className="flex justify-between items-center p-8 border-b border-slate-100 dark:border-white/5 shrink-0">
                    <div className="text-xl font-bold text-slate-900 dark:text-white leading-tight">{title}</div>
                    <button
                        onClick={onClose}
