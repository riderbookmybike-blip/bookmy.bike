...
export const AppHeaderShell: React.FC<AppHeaderShellProps> = ({
    ...
    const bgClass = (transparentAtTop && !scrolled)
        ? "bg-transparent border-b-transparent"
        : "bg-white/70 dark:bg-slate-950/70 backdrop-blur-md border-b border-slate-200 dark:border-white/10 shadow-sm";

    return (
        <>
            <header className={`sticky top-0 z-50 h-16 w-full flex items-center transition-all duration-300 ${bgClass} ${className}`}>
                <div className="max-w-[1400px] mx-auto px-6 w-full flex items-center justify-between h-full">
                    ...
                    <div className="hidden md:flex flex-1 justify-center items-center h-full mx-8">
                        {center}
                    </div>
