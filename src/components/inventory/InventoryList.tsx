...
export default function InventoryList({ items, onSelect, selectedId }: InventoryListProps) {
    ...
    const metrics = (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-8 mb-4">
            {[
                { label: 'Total Units', value: items.reduce((acc, i) => acc + i.totalStock, 0), icon: Box, color: 'text-blue-500' },
                { label: 'Available', value: items.reduce((acc, i) => acc + i.available, 0), icon: ShieldCheck, color: 'text-emerald-500' },
                { label: 'Reserved', value: items.reduce((acc, i) => acc + i.reserved, 0), icon: Zap, color: 'text-amber-500' },
                { label: 'Allotted', value: items.reduce((acc, i) => acc + i.allotted, 0), icon: BarChart4, color: 'text-rose-500' },
            ].map((stat, i) => (
