            ...
            <div className="p-6 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-slate-950 flex-shrink-0">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{item.brand} {item.model}</h1>
                        <p className="text-gray-600 dark:text-slate-400">{item.variant} - {item.color}</p>
                    </div>
            ...
            <div className="p-8">
                ...
                <div className="grid grid-cols-4 gap-6 mb-8">
                    <div className="p-4 bg-gray-50 dark:bg-slate-950 rounded-xl border border-gray-200 dark:border-white/10">
                        <div className="text-xs text-gray-500 dark:text-slate-400 uppercase font-bold mb-1">Total Physical</div>
                    ...
                    <div className="p-4 bg-yellow-50 dark:bg-yellow-500/10 rounded-xl border border-yellow-200 dark:border-yellow-500/20">
                        <div className="text-xs text-yellow-700 dark:text-yellow-300 uppercase font-bold mb-1">Soft Locked</div>
                        <div className="text-2xl font-bold text-yellow-900 dark:text-yellow-200">{item.reserved}</div>
                        <div className="text-[10px] text-yellow-600 dark:text-yellow-300 mt-1">Pending Confirmation</div>
                    </div>
                    <div className="p-4 bg-red-50 dark:bg-red-500/10 rounded-xl border border-red-200 dark:border-red-500/20">
                        <div className="text-xs text-red-700 dark:text-red-300 uppercase font-bold mb-1">Hard Locked</div>
                                ...
                                {relatedVins.map(v => (
                                    <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-white/5">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${v.status === 'AVAILABLE' ? 'bg-green-100 dark:bg-green-500/10 text-green-700 dark:text-green-400' :
                                                v.status === 'ASSIGNED' ? 'bg-indigo-100 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-slate-400'
                                                }`}>
                                                {v.status}
                                            </span>
                                            {v.bookingId && <div className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Ref: {v.bookingId}</div>}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
