...
export default function DebugPortal() {
    const ALLOWED_PHONE = '9820760596';
    ...
    useEffect(() => {
        const checkAccess = async () => {
            ...
            const phone =
                ...
                normalizeIndianPhone((user?.user_metadata as any)?.phone || '');
            setIsAllowed(phone === ALLOWED_PHONE);
        };
        checkAccess();
    }, []);
    ...
    return (
        <div
            className={`fixed bottom-0 right-0 z-[9999] w-[90vw] max-w-[1400px] transition-all duration-300 ${isMinimized ? 'h-14' : 'h-[85vh]'} bg-slate-950/95 backdrop-blur-xl border-l border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden rounded-tl-3xl`}
        >
            {/* Header */}
                ...
                <>
                    ...
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-2 no-scrollbar font-mono">
                        {filteredLogs.map(log => (
                            <div
                                key={log.id}
                                className={`p-3 rounded-xl border bg-slate-900/40 transition-all hover:bg-slate-900/60 group animate-in slide-in-from-right-4 duration-300 ${
                                    log.status === 'ERROR' ? 'border-rose-500/20 bg-rose-500/5' : 'border-white/5'
                                }`}
                            >
                                ...
                                <div
                                    className={`text-[11px] font-bold tracking-tight mb-2 ${
                                        log.status === 'ERROR' ? 'text-rose-400' : 'text-slate-200'
                                    }`}
                                >
                                    {log.message}
                                </div>
                                ...
                                {log.error && JSON.stringify(log.error) !== '{}' && (
                                    <div className="mb-2">
                                        <div className="text-[8px] uppercase tracking-widest text-rose-600 mb-1">
                                            Error Trace
                                        </div>
