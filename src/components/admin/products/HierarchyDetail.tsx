    ...
    if (selectedVariant) {
        return (
            <div className="h-full flex flex-col bg-white">
                {/* Breadcrumb */}
                <div className="p-4 border-b border-gray-200 flex items-center gap-2 text-sm text-gray-500 bg-gray-50">
                    ...
                    <ChevronRight size={14} />
                    <span className="font-bold text-gray-900">{selectedVariant.name}</span>
                </div>
                ...
                <div className="p-6 border-b border-gray-200">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-gray-500 text-sm mt-1">{selectedVariant.variants.length} Configurations (SKUs)</p>
                        </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedVariant.variants.map(sku => (
                        <div key={sku.id} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors">
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="font-mono text-xs text-gray-500 mb-1">{sku.sku}</div>
                                    <div className="font-bold text-gray-800">{type === 'VEHICLE' ? sku.color : sku.size || sku.duration || 'Standard'}</div>
                                    <div className="text-xs text-gray-600 mt-2">{sku.label}</div>
                                </div>
                                <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${sku.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                    {sku.status}
                                </span>
                            </div>
                        </div>
            ...
            <div className="h-full flex flex-col bg-white">
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-200">
                    ...
                    <div className="space-y-2">
                        {Array.from(selectedModel.children.values()).map(variant => (
                            <div
                                ...
                                className="group flex items-center justify-between p-4 border border-gray-200 rounded-lg cursor-pointer hover:border-blue-400 transition-all hover:bg-blue-50"
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">{variant.variants.length} SKUs defined</p>
                                    </div>
                                </div>
        ...
        <div className="h-full flex flex-col bg-white">
            ...
            <div className="p-6 border-b border-gray-200 bg-gray-50">
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-gray-200 flex items-center justify-center text-lg font-bold text-gray-700">
                            {brandNode.name[0]}
                        </div>
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{brandNode.name}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Array.from(brandNode.children.values()).map(model => (
                        <div
                            ...
                            className="bg-white border border-gray-200 p-4 rounded-xl cursor-pointer hover:shadow-md hover:border-gray-300 transition-all group"
                        >
