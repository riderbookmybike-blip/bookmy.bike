...
import { ProductTemplateGallery, CatalogTemplate } from '@/components/admin/ProductTemplateGallery';
...
interface UnifiedTemplateStudioProps {
    ...
    productTemplates: CatalogTemplate[];
}

export function UnifiedTemplateStudio({
    dashboardTemplates,
    dashboardAssignments,
    productTemplates
}: UnifiedTemplateStudioProps) {
    const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'PRODUCT' | 'SIDEBAR'>('DASHBOARD');
        ...
        <div className="space-y-8">
            ...
            <div className="flex gap-2 border-b border-slate-200 dark:border-white/5 pb-1">
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl text-xs font-black uppercase tracking-wider border-b-2 transition-all ${activeTab === 'DASHBOARD'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl text-xs font-black uppercase tracking-wider border-b-2 transition-all ${activeTab === 'PRODUCT'
                            ? 'border-indigo-600 text-indigo-600 bg-indigo-50 dark:bg-indigo-900/10'
                            : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800'
                        }`}
                >
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl text-xs font-black uppercase tracking-wider border-b-2 transition-all ${activeTab === 'SIDEBAR'
                            ? 'border-indigo-600 text-indigo-600 bg-indigo-50 dark:bg-indigo-900/10'
                            : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800'
                        }`}
                >
            ...
            <div className="min-h-[500px]">
                ...
                {activeTab === 'SIDEBAR' && (
                    <div className="text-center py-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <span className="inline-block mt-4 px-3 py-1 bg-indigo-100 text-indigo-600 rounded-lg text-[10px] font-bold uppercase tracking-wider">Coming Soon</span>
                    </div>
                )}
            </div>
        </div>
