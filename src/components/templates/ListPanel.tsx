...
export default function ListPanel({
    ...
    const displayRows = hasData ? filteredData : showPlaceholders ? Array.from({ length: 10 }).map((_, i) => ({
        id: i,
        col1: `Item Name ${i + 1}`,
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-slate-900/40 backdrop-blur-xl relative z-10 transition-colors duration-500">
            ...
            <div className={`flex-1 mx-6 mb-8 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-[32px] shadow-sm flex flex-col overflow-hidden`}>
                <div className="overflow-x-auto flex-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-200 dark:scrollbar-thumb-white/10">
                    <table className={`${tight ? 'w-fit' : 'w-full'} text-left text-sm border-separate border-spacing-0 min-w-full relative`}>
                        <thead className="bg-slate-50/80 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/10 sticky top-0 z-20 backdrop-blur-xl">
                            <tr>
                                {showIndex && (
                                    <th className="p-4 w-12 border-b border-slate-200 dark:border-white/10 pl-6 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">#</th>
                                )}
                                {showSelection && (
                                ...
                                {onQuickAction && (
                                    <th className="p-4 border-b border-slate-200 dark:border-white/10 text-right pr-6 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] w-24">Actions</th>
                                )}
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                Array.from({ length: 8 }).map((_, i) => (
                                    <tr key={`skeleton-${i}`} className="animate-pulse">
                                        <td className="p-4"><div className="h-4 w-4 bg-slate-100 dark:bg-white/5 rounded-lg" /></td>
                                        {columns.map((col, idx) => (
                                            <td key={idx} className="p-4"><div className="h-4 bg-slate-100 dark:bg-white/5 rounded-lg w-3/4" /></td>
                                        ))}
                                    </tr>
                                ))
                                ...
                                displayRows.map((item, rowIdx) => (
                                    <tr
                                        ...
                                        ${selectedId === item.id
                                                ? 'bg-indigo-600/5 dark:bg-indigo-500/5'
                                                : ''
                                            }
                                    `}
                                    >
                                            ...
                                            >
                                                {col.render ? (
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        {col.icon && (
                                                            <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-white/5 flex items-center justify-center p-1.5 shadow-sm border border-slate-100 dark:border-white/10 transition-transform group-hover:scale-110">
                                                                {React.createElement(col.icon, { size: 16, className: 'text-slate-700 dark:text-slate-300' })}
                                                            </div>
                                                        )}
                                                        <div>
                                                    ...
                                                    <span className={`inline-flex items-center px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest border ${getBadgeClass(item[col.key] || item.status)} shadow-sm`}>
                                                        {getDotColor(item[col.key]) && (
                                                            <span className="flex h-1.5 w-1.5 mr-1.5 relative">
                                                                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${getDotColor(item[col.key])}`}></span>
                                                                <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ${getDotColor(item[col.key])}`}></span>
                                                            </span>
                                                        )}
                                                    ...
                                                    <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 overflow-hidden flex items-center justify-center p-1 group-hover:scale-110 transition-transform">
                                                        {item[col.key] ? (
                                                            <img src={item[col.key]} alt="" className="w-full h-full object-contain" />
                                                        ) : (
                                        ...
                                        {onQuickAction && (
                                            <td className="p-4 text-right pr-6 whitespace-nowrap">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); onQuickAction('edit', item); }}
                                ...
                                <tr>
                                    <td colSpan={columns.length + 1} className="p-16 text-center">
                                        <div className="flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
                                            ...
                                            <h3 className="text-lg font-black text-slate-900 dark:text-white mb-2 italic tracking-tighter uppercase">No records found</h3>
