 ...
 */
export const DetailWorkspace: React.FC<DetailWorkspaceProps> = ({ leftContent, rightContent, className = '' }) => {
    return (
        <div className={`flex gap-6 w-full h-full ${className}`}>
            {/* LEFT: Primary Work Area (65-70%) */}
            <div className="flex-[7] min-w-0 overflow-auto">{leftContent}</div>

            {/* RIGHT: Context Intelligence (30-35%) */}
            <div className="flex-[3] min-w-0 overflow-auto">{rightContent}</div>
        </div>
    );
};
