...
export default function AttributeInput({ attr, value, onChange, placeholder, className }: AttributeInputProps) {
    ...
    const baseClass =
        className ||
        'bg-transparent font-bold text-lg outline-none w-full border-b border-transparent focus:border-indigo-500 py-1';
            ...
            <div className="w-full space-y-3 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-white/5">
                ...
                <div className="space-y-2">
                    {items.map((it: any, idx: number) => (
                        <div
                            key={idx}
                            className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-white/5 group"
                        >
    ...
    const displayValue =
        attr.type === 'number' ? (value === '' || value === null || value === undefined ? '' : value) : value || '';
