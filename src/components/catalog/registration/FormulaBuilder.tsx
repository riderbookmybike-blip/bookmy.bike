...
const SortableComponent = ({ component, onChange, onDelete, readOnly, availableTargets, inheritedContext }: {
...
export default function FormulaBuilder({ components, onChange, readOnly = false, availableTargets = [], inheritedContext }: FormulaBuilderProps) {
    ...
    const handleAdd = (type: ComponentType | 'FUEL_SPLIT', basis?: 'PREVIOUS_TAX_TOTAL' | 'TARGET_COMPONENT') => {
        const newComp: FormulaComponent = {
            ...
            label: basis === 'PREVIOUS_TAX_TOTAL' ? 'New Cess / Surcharge' : basis === 'TARGET_COMPONENT' ? 'Component Specific Charge' : ('New ' + (type === 'FUEL_SPLIT' ? 'Split' : type.toLowerCase().replace('_', ' '))),
        ...
        <div className={`group border border-white/20 rounded-2xl bg-white/60 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300 ring-1 ${ringColor} overflow-hidden ${borderColor}`}>
            ...
            <div className={`flex items-center gap-4 px-4 py-3 cursor-pointer group/header ${headerColor} rounded-t-2xl border-b ${borderColor} transition-colors ${isDragging ? 'bg-blue-50/50' : ''}`} onClick={() => setExpanded(!expanded)}>
                ...
                <div className="flex items-center gap-2 flex-1">
                    ...
                    {effectiveReadOnly ? (
                        <span className="font-extrabold text-sm text-slate-900 dark:text-white tracking-tight">{component.label}</span>
                    ) : (
                        <input
                            ...
                            placeholder={isSlab ? "Slab Name (e.g. MV Tax Table)" : component.type === 'CONDITIONAL' ? "Condition Name" : component.type === 'SWITCH' ? "Split Name" : isTaxOnTax ? "Cess Name" : "Charge Label"}
                        />
                    )}
                </div>
                            ...
                            <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                                ...
                                {(component.type === 'PERCENTAGE' || component.type === 'FIXED') && (
                                    <div className="flex items-center gap-1 mr-2 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-white/5">
                                        <label className="text-[9px] font-black text-slate-400 uppercase px-1.5">Basis</label>
                                        <select
                                    ...
                                    <div className="flex items-center gap-1 mr-2 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-white/5">
                                        ...
                                        >
                                            ...
                                            {availableTargets
                                                ...
                                                .map(t => (
                                                    <option key={t.id} value={t.id}>{t.label || '(Unnamed)'}</option>
                                                ))
                                            }
                                        </select>
                                    </div>
                                ...
                                {(component.type === 'PERCENTAGE' || component.type === 'FIXED' || component.type === 'SLAB') && (
                                    <div className="flex items-center gap-1 mr-2 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-white/5">
                                        <select
                                            ...
                                            value={component.variantTreatment || (component.type === 'SLAB' ? 'PRO_RATA' : 'NONE')}
            ...
            {expanded && (
                <div className="p-3 bg-white/40 dark:bg-slate-900/60 backdrop-blur-md">
                    ...
                    {component.type === 'PERCENTAGE' && (
                        <div className="flex items-center gap-4">
                            <div className="flex-1">
                                <label className="block text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Percentage by Fuel (%)</label>
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    {['PETROL', 'DIESEL', 'EV', 'CNG'].map(fuel => {
                                        ...
                                        const val = component.fuelMatrix?.[fuel as keyof typeof component.fuelMatrix] ??
                                            (fuel === 'PETROL' ? component.percentage : 0) ?? 0;

                                        return (
                                            <div key={fuel}>
                                                <label className="text-[10px] text-slate-600 dark:text-slate-300 font-extrabold block mb-1 uppercase tracking-tighter italic">{fuel}</label>
                                                <div className="relative group/input">
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            const newMatrix = {
                                                                PETROL: component.fuelMatrix?.PETROL ?? component.percentage ?? 0,
                                                        ...
                                                        step="0.001"
                                                    />
                                                    <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-black text-blue-400/50 dark:text-blue-400/40">%</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                    ...
                    {component.type === 'FIXED' && (
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Fixed Amount (₹)</label>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {['PETROL', 'DIESEL', 'EV', 'CNG'].map(fuel => {
                                    ...
                                    const val = component.fuelMatrix?.[fuel as keyof typeof component.fuelMatrix] ??
                                        (fuel === 'PETROL' ? component.amount : 0) ?? 0;

                                    return (
                                        <div key={fuel}>
                                            <label className="text-[10px] text-slate-600 dark:text-slate-300 font-extrabold block mb-1 uppercase tracking-tighter italic">{fuel}</label>
                                            <div className="relative">
                                                <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[10px] font-black text-green-500 dark:text-green-400 opacity-60">₹</span>
                                                <input
                                                    type="number"
                                                    value={val}
                                                    onChange={(e) => {
                                                        const newVal = parseFloat(e.target.value);
                                                        const newMatrix = {
                                                            PETROL: component.fuelMatrix?.PETROL ?? component.amount ?? 0,
                                                            DIESEL: component.fuelMatrix?.DIESEL ?? 0,
                                                            EV: component.fuelMatrix?.EV ?? 0,
                                                            CNG: component.fuelMatrix?.CNG ?? 0,
                                                            ...component.fuelMatrix
                    ...
                    {component.type === 'CONDITIONAL' && (
                        <div className="space-y-3">
                            ...
                            <div className="flex gap-2 items-center bg-white/40 dark:bg-slate-900/40 p-2 border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm">
                                <span className="text-[10px] font-black text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 px-3 py-1 rounded-xl tracking-widest uppercase italic">IF</span>
                                <select
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="border border-green-200 dark:border-green-900/30 bg-green-50/30 dark:bg-green-900/10 rounded-2xl p-2 flex flex-col h-full relative group">
                                    ...
                                    <div className="flex items-center justify-between mb-2 pl-3">
                                        <span className="text-[10px] font-black text-green-700 dark:text-green-400 uppercase tracking-widest italic">Then (True)</span>
                                    </div>
                                ...
                                <div className="border border-red-200 dark:border-red-900/30 bg-red-50/30 dark:bg-red-900/10 rounded-2xl p-2 flex flex-col h-full relative group">
                                    ...
                                    <div className="flex items-center justify-between mb-2 pl-3">
                                        <span className="text-[10px] font-black text-red-700 dark:text-red-400 uppercase tracking-widest italic">Else (False)</span>
                                    </div>
                                    <div className="flex-1 pl-2">
                                        <FormulaBuilder
                    ...
                    {component.type === 'SLAB' && (
                        <div>
                            <div className="flex items-center gap-2 mb-3">
                                <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Universal Tax Table</span>
                            ...
                            <div className="border border-white/40 dark:border-white/5 rounded-3xl overflow-hidden bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm shadow-inner ring-1 ring-black/5 mt-4">
                                <table className="w-full text-sm text-left border-collapse">
                                    <thead className="bg-slate-100/50 dark:bg-white/5 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-black tracking-widest border-b border-black/5 dark:border-white/5">
                                        <tr>
                                            {!inheritedContext?.fuelType && <th className="px-4 py-3 w-[25%]">Applicable Fuels</th>}
                                    ...
                                    <tbody className="divide-y divide-black/5 dark:divide-white/5">
                                        {(component.ranges || []).map((range, rIdx) => {
                                            ...
                                            return (
                                                <tr key={range.id} className="group hover:bg-white/50 dark:hover:bg-white/5 transition-all duration-200">
                                                    {!inheritedContext?.fuelType && (
                                                        <td className="px-4 py-3">
                                                            <div className="flex gap-1.5 flex-wrap">
                                                                {['PETROL', 'DIESEL', 'EV', 'CNG'].map(fuel => {
                                                                    const active = range.applicableFuels?.includes(fuel as any);
                                                                    return (
                                                                        <button
                                                                            key={fuel}
                                                                            onClick={() => !effectiveReadOnly && toggleFuel(fuel)}
                                                                            className={`text-[10px] font-black px-2 py-1 rounded-lg border-2 transition-all duration-300 ${active
                                                                                ? fuel === 'EV' ? 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-500/30' :
                                                                                    fuel === 'PETROL' ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-500/30' :
                                                                                        fuel === 'CNG' ? 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30' :
                                                                        ...
                                                                        >
                                                                            {fuel.substring(0, 1)}
                                                                            <span className="hidden xl:inline ml-0.5">{fuel.substring(1).toLowerCase()}</span>
                                                                        </button>
                                                                    )
                                                                })}
                                                            </div>
                                                    ...
                                                    <td className="px-3 py-3">
                                                        <select
                                                            ...
                                                            value={range.slabBasis || component.slabVariable || 'ENGINE_CC'}
                                                            onChange={(e) => updateRange({ slabBasis: e.target.value as any })}
                                                            disabled={effectiveReadOnly}
                                                        >
                                                            <option value="ENGINE_CC">Engine CC</option>
                                                            <option value="KW_RATING">EV KW Rating</option>
                                                    ...
                                                    <td className="px-3 py-3">
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                updateRange({ max: val === '' ? null : parseFloat(val) });
                                                            }}
                                                            className="w-full bg-white/50 dark:bg-slate-800 border border-slate-200 dark:border-white/10 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 rounded-lg px-2 py-1.5 text-xs font-black text-slate-900 dark:text-white"
                                                            placeholder="∞"
                                                            readOnly={effectiveReadOnly}
                                                    ...
                                                    <td className="px-3 py-3 text-center border-l border-orange-200/20 bg-orange-500/[0.02]">
                                                        <input
                                                            ...
                                                            onChange={(e) => updateRange({ cessPercentage: e.target.value === '' ? undefined : parseFloat(e.target.value) })}
                                                    ...
                                                    {!effectiveReadOnly && (
                                                        <td className="px-3 py-3 text-right">
                                                            <button
                                                                onClick={() => {
                                                                    const newRanges = (component.ranges || []).filter((_, i) => i !== rIdx);
                                                                    updateField('ranges', newRanges);
                                                                }}
                    ...
                    {component.type === 'SWITCH' && (
                        <div className="space-y-3">
                            <div className="flex items-center gap-2 mb-2">
                                <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Split By:</span>
                                <select
                                    ...
                                    <div key={caseItem.id} className="relative group">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest">Case: {caseItem.label}</span>
                                                <span className="text-[10px] bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-lg font-black italic">"{caseItem.matchValue}"</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-white/40 dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl p-2 shadow-sm">
                                            <FormulaBuilder
                                                ...
                                                inheritedContext={{
                                                    ...inheritedContext,
                                                    fuelType: component.switchVariable === 'FUEL_TYPE' ? caseItem.matchValue : inheritedContext?.fuelType
                                                }}
                                            />
                                        </div>
                                    </div>
