...
import { Calculator, Plus, Shield, Zap, Lock, Banknote, Clock } from 'lucide-react';
        ...
        <div className="space-y-12 pb-24">
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-3 px-4">
                    ...
                    <div>
                        <h3 className="text-lg font-black tracking-tighter italic uppercase text-slate-900 dark:text-white leading-none">
                            IDV Logic
                        </h3>
                        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                            Insured Declared Value derivation
                        </p>
                    </div>
                </div>
                ...
                <div className="mx-4 p-6 bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] border border-white/60 dark:border-white/5 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:shadow-2xl transition-all duration-500">
                    <div className="flex-1">
                        <label className="block text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">
                            Depreciation Percentage of Ex-Showroom
                        </label>
                        <div className="flex items-center gap-4">
                            <div className="relative w-48 group">
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-black text-blue-500 dark:text-blue-400 opacity-60">
                                    %
                                </span>
                                <input
                                    type="number"
                            ...
                            <div className="flex-1 p-4 bg-blue-50/50 dark:bg-blue-500/10 rounded-2xl border border-blue-100 dark:border-blue-500/20">
                                <p className="text-[11px] font-black text-blue-700 dark:text-blue-300 uppercase italic tracking-tight">
                                    Standard IDV is typically 95% for new vehicles.
                                </p>
                                <p className="text-[10px] font-medium text-blue-400 dark:text-blue-500 mt-1 uppercase tracking-widest">
                                    Current Logic: {idvPercentage}% of Ex-Showroom
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <section className="space-y-6">
                <div className="flex items-center justify-between px-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-black tracking-tighter italic uppercase text-slate-900 dark:text-white leading-none flex items-center gap-2">
                                Own Damage (OD)
                                <select
                                    value={config.od.default}
                                    onChange={e => handleTenureChange('od', Number(e.target.value))}
                                    disabled={readOnly}
                                    className="ml-2 px-2 py-0.5 bg-blue-600 text-white text-[9px] font-black rounded-full tracking-widest border-0 outline-none cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {[1, 3, 5].map(y => (
                        ...
                        <div>
                            <h3 className="text-lg font-black tracking-tighter italic uppercase text-slate-900 dark:text-white leading-none flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    disabled={readOnly}
                                    className="ml-2 px-2 py-0.5 bg-indigo-600 text-white text-[9px] font-black rounded-full tracking-widest border-0 outline-none cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {[1, 5].map(y => (
                                        <option key={y} value={y}>
                                            {y} YR
                            ...
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                Fixed liability based on engine capacity
                            </p>
                        </div>
                    </div>
                    {!readOnly && (
                ...
                <div className="flex items-center justify-between px-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-black tracking-tighter italic uppercase text-slate-900 dark:text-white leading-none flex items-center gap-2">
                                Add-on Covers
                                <span className="flex items-center gap-1 px-2 py-0.5 bg-amber-500 text-white text-[9px] font-black rounded-full tracking-widest">
                                    <Clock size={10} /> {config.od.default} YR
                                </span>
                                <span className="text-[8px] font-medium text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full">
                                    = OD Tenure
                                </span>
                            </h3>
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                Zero Dep, RTI, and other consumables
                            </p>
                        </div>
                    </div>
                    {!readOnly && (
