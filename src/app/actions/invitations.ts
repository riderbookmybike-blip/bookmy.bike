...
export async function createInvite(prevState: any, formData: FormData) {
    ...
    const membershipRole = (membership as any)?.role || '';
    if (!membership || !['DEALER_OWNER', 'DEALER_ADMIN', 'SUPER_ADMIN'].includes(membershipRole)) {
        return { success: false, message: 'Insufficient permissions' };
    }

    const { error } = await (adminClient as any).from('invitations').insert({
        tenant_id: tenantId,
        email,
        role,
