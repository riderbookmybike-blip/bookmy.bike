...
export default function BookingsPage() {
    ...
    const stats = [
        ...
        { label: 'In Delivery', value: bookings.filter(b => b.status === 'READY_FOR_DELIVERY').length, icon: Truck, color: 'blue' as const },
        { label: 'Payments', value: bookings.filter(b => b.paymentStatus === 'PAID').length, icon: CreditCard, color: 'emerald' as const, trend: '80%' },
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{booking.customerPhone}</div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-100 dark:border-white/5">
                                        <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Order Reference</th>
                                        <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Customer Persona</th>
                                        <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Vehicle Unit</th>
                                        <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Contract Value</th>
                                        <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Fulfillment Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredBookings.map((booking) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-6">
                                                <div>
                                                    ...
                                                    <div className="text-[10px] font-bold text-slate-400">{booking.customerPhone}</div>
                                                </div>
                                            </td>
                                            <td className="p-6">
                ...
                <div className="h-full flex flex-col bg-white dark:bg-[#0b0d10] border-r border-slate-200 dark:border-white/5 w-full">
                    <div className="p-6 border-b border-slate-100 dark:border-white/5 space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={() => setSelectedBooking(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all text-slate-400">
                                <LayoutGrid size={18} />
                            </button>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
                        {filteredBookings.map((booking) => (
                            <button
                                key={booking.id}
                                onClick={() => setSelectedBooking(booking)}
                                className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 group ${selectedBooking?.id === booking.id
                            ...
                            >
                                ...
                                <div className={`text-[9px] font-bold ${selectedBooking?.id === booking.id ? 'text-white/80' : 'text-slate-500'} flex items-center gap-2`}>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white dark:bg-white/5 rounded-[2.5rem] p-10 border border-slate-200 dark:border-white/10 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-4 mb-8">
                                        ...
                                        <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Vehicle Specifications</h3>
                                    </div>
                                    <div className="space-y-6">
                                        <div className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-white/5">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Model Name</span>
                                            <span className="text-sm font-black text-slate-900 dark:text-white uppercase italic">{selectedBooking.vehicleDetails?.name}</span>
                                        </div>
                                        <div className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-white/5">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Chassis Number</span>
                                            <span className="text-sm font-bold text-slate-900 dark:text-white font-mono">{selectedBooking.vehicleDetails?.chassis || 'PENDING'}</span>
                                        </div>
                                        <div className="flex justify-between items-center py-2">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Delivery Schedule</span>
                            ...
                            <div className="bg-white dark:bg-white/5 rounded-[2.5rem] p-10 border border-slate-200 dark:border-white/10 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-4 mb-8">
                                        ...
                                        <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Commercial Settlement</h3>
                                    </div>
                                    <div className="space-y-6">
                                        <div className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-white/5">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Contract Value</span>
                                            <span className="text-2xl font-black text-emerald-600">₹{selectedBooking.totalAmount?.toLocaleString()}</span>
                                        </div>
                                        <div className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-white/5">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Payment Status</span>
                                            <span className={`text-[10px] font-black px-3 py-1 rounded-full border ${selectedBooking.paymentStatus === 'PAID' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-amber-500/10 border-amber-500/20 text-amber-500'} uppercase tracking-widest`}>
                                                {selectedBooking.paymentStatus || 'UNPAID'}
                                            </span>
                                        </div>
                                        <div className="flex justify-between items-center py-2">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Booking Source</span>
                            ...
                            <div className="lg:col-span-2 bg-white dark:bg-white/5 rounded-[2.5rem] p-10 border border-slate-200 dark:border-white/10 shadow-sm relative overflow-hidden group">
                                <div className="flex items-center gap-4 mb-10">
                                    ...
                                    <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Included Accessories Inventory</h3>
                                </div>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    {selectedBooking.accessories?.map((acc: string, idx: number) => (
                                        <div key={idx} className="flex flex-col gap-3 p-6 bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl group/acc transition-all hover:border-purple-500/30">
