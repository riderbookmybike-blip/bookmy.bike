...
import { ShoppingBag, Truck, FileCheck, ShieldCheck, ArrowRight, Loader2, Landmark, CreditCard } from 'lucide-react';
...
export default function SalesOrdersPage() {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            // Apply stage filtering if present
            const filteredData = stageFilter
                ? (data as any[]).filter(
                      o => o.operationalStage === stageFilter || (stageFilter === 'BOOKING' && o.status === 'BOOKED')
                  )
                : data;

            setOrders(filteredData);
            if (filteredData.length > 0 && !selectedOrder) {
    ...
    const stages = [
        {
            id: 'BOOKING',
            label: 'Booking',
            count: orders.filter(o => o.status === 'BOOKED' || o.status === 'DRAFT').length,
            icon: ShoppingBag,
            color: 'text-blue-600',
            bg: 'bg-blue-50',
        },
        {
            id: 'PAYMENT',
            label: 'Payment',
            count: orders.filter(o => o.operationalStage === 'PAYMENT').length,
            icon: CreditCard,
        ...
        },
        {
            id: 'FINANCE',
            label: 'Finance',
            count: orders.filter(o => o.operationalStage === 'FINANCE').length,
            icon: Landmark,
        ...
        },
        {
            id: 'ALLOTMENT',
            label: 'Allotment',
            count: orders.filter(o => o.operationalStage === 'ALLOTMENT').length,
            icon: FileCheck,
        ...
        },
        {
            id: 'COMPLIANCE',
            label: 'Compliance',
            count: orders.filter(o => o.operationalStage === 'COMPLIANCE').length,
            icon: ShieldCheck,
            color: 'text-emerald-600',
            bg: 'bg-emerald-50',
        },
        {
            id: 'DELIVERED',
            label: 'Delivery',
            count: orders.filter(o => o.operationalStage === 'DELIVERED').length,
            icon: Truck,
        ...
        <div className="flex flex-col h-[calc(100vh-140px)] gap-6">
            ...
            <div className="grid grid-cols-6 gap-4">
                {stages.map((stage, i) => (
                    <div
                        key={i}
                        className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex items-center gap-4"
                    >
                    ...
                    {selectedOrder ? (
                        <>
                            ...
                            <div className="bg-white dark:bg-slate-900 px-8 py-6 border-b border-slate-100 dark:border-white/10">
                                <div className="flex items-center justify-between relative">
                                    ...
                                    {['Booking', 'Allotment', 'Compliance', 'Delivery'].map((step, i) => {
                                        const stageList = ['BOOKING', 'ALLOTMENT', 'COMPLIANCE', 'DELIVERED'];
                                        const currentStageIndex = stageList.indexOf(
                                            selectedOrder.operationalStage || 'BOOKING'
                                        );
                                        ...
                                        return (
                                            <div key={i} className="relative z-10 flex flex-col items-center gap-2">
                                                ...
                                                <span
                                                    className={`text-[9px] font-black uppercase tracking-widest ${isCurrent ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}
                                                >
                                                    {step}
                                                </span>
                                            </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                            ...
                            <p className="text-slate-400 dark:text-slate-500 text-sm font-medium mt-1 max-w-xs">
                                Choose a sales order to track its fulfillment progress across payment, allotment, and
                                delivery stages.
                            </p>
                        </div>
                    )}
