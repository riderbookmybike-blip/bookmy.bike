...
import { Palette, Grid3X3, Layers, Star, Loader2, Upload, Image as ImageIcon } from 'lucide-react';
...
export default function SKUStepV2({ model, variants, colours, skus, onUpdate }: SKUStepProps) {
    ...
    // Set a SKU as primary for its variant
                        ...
                        <table className="w-full text-sm">
                                ...
                                <tr className="border-b border-slate-100 dark:border-white/5">
                                    ...
                                    {variants.map((variant: any) => {
                                        ...
                                        const variantActiveSkus = skus.filter(
                                            s => (s as any)[variantFkField] === variant.id && s.status === 'ACTIVE'
                                        );
                                        const primarySku =
                                            variantActiveSkus.find(s => s.is_primary) || variantActiveSkus[0];
                                        const hex = primarySku ? getSkuSwatch(primarySku) : null;
                                        return (
                                            <th key={variant.id} className="px-3 py-4 text-center whitespace-nowrap">
                            ...
                            <tbody>
                                {sortedColours.map(colour => {
                                    const coloursForRow = variants.filter((v: any) => getSkuForCell(v.id, colour.id));
                                            ...
                                            <td className="px-5 py-3 sticky left-0 bg-white dark:bg-slate-900 z-10 min-w-[180px] max-w-[220px]">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    >
                                                        ...
                                                        <span className="break-words leading-tight">{colour.name}</span>
                                                    </span>
                                                    <button
                                                            ...
                                                            <div className="flex items-center justify-center gap-1.5">
                                                                {isBusy ? (
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <button
                                                                            onClick={() =>
                                                                                handleToggleSku(variant.id, colour)
                                                                            }
                                                                            title={sku ? 'Remove SKU' : 'Create SKU'}
                                                            ...
                                                            {sku && (
                                                                ...
                                                                >
                                                                    {sku.primary_image ? (
                                                                        <>
                                                                            <img
                                                                                src={getProxiedUrl(sku.primary_image)}
