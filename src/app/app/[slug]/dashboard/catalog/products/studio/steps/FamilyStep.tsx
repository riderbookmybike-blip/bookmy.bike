        ...
        try {
            ...
            const payload = {
                ...
                specs: {
                    ...
                    engine_cc: (formData.specs as any).engine_cc || (formData.specs as any).engine_capacity,
                    fuel_capacity:
                        (formData.specs as any).fuel_capacity ||
                        (formData.specs as any).fuel_tank_capacity ||
                        (formData.specs as any).tank_capacity ||
                        (formData.specs as any).fuel_tank,
            ...
            if (data) {
                ...
                if (oldValues) {
                    ...
                    if (oldValues.engine_cc !== (data.specs as any)?.engine_cc)
                        changes.push(`CC: ${oldValues.engine_cc || '—'} → ${(data.specs as any)?.engine_cc || '—'}`);
                    if (oldValues.price_base !== data.price_base)
                        changes.push(`Price: ₹${oldValues.price_base} → ₹${data.price_base}`);
                    if (oldValues.hsn_code !== data.hsn_code)
                        changes.push(`HSN: ${oldValues.hsn_code || '—'} → ${data.hsn_code || '—'}`);
            ...
            <div className="space-y-6">
                    ...
                    {families &&
                        families.map((fam: any) => {
                            ...
                            return (
                                <div
                                    key={fam.id}
                                    className={`group relative p-5 rounded-[2.5rem] border-2 transition-all duration-300 text-left overflow-hidden flex flex-col justify-between gap-4 min-h-[300px] hover:-translate-y-1 ${
                                ...
                                >
                                    <div className="w-full aspect-[4/3] rounded-3xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center overflow-hidden border border-slate-100 dark:border-white/5 group-hover:border-indigo-100 transition-all relative">
                                        {specs?.primary_image ? (
                                            <>
                                                <img
                                                    src={getProxiedUrl(specs.primary_image)}
                                                    alt={fam.name}
                                            ...
                                            <button
                                                onClick={e => {
                                                    ...
                                                    setFormData({
                                                        ...
                                                        specs: {
                                                            ...specs,
                                                            engine_cc: specs?.engine_cc || specs?.engine_capacity || '',
                                                            fuel_capacity:
                                                                specs?.fuel_capacity ||
                                                                ...
                                                                specs?.fuel_tank ||
                                                                '',
                                                            mileage: specs?.mileage || specs?.arai_mileage || '',
                                                        },
                                    ...
                                    <div className="w-full">
                                        <h4 className="font-black text-slate-900 dark:text-white uppercase italic text-xl leading-none mb-1">
                                            {fam.name}
                                        </h4>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                >
                                                    ...
                                                    <span className="text-[10px] font-bold">
                                                        {specs?.gallery?.length || 0}
                                                    </span>
                                                </div>
                                                {specs?.video_urls?.length > 0 && (
                                                    <div
                                                        className="w-6 h-6 rounded-md bg-rose-50 dark:bg-rose-900/20 text-rose-500 flex items-center justify-center border border-rose-100 dark:border-rose-500/20"
                                                        title="Videos Available"
                                                    >
                                                        <Video size={12} fill="currentColor" />
                                                    </div>
                                                )}
                                                {specs?.pdf_urls?.length > 0 && (
                                                    <div
                                                        className="w-6 h-6 rounded-md bg-blue-50 dark:bg-blue-900/20 text-blue-500 flex items-center justify-center border border-blue-100 dark:border-blue-500/20"
                ...
                >
                    <div className="flex flex-col gap-6">
                        <div className="flex items-end justify-between">
                            ...
                            <div className="flex items-center gap-2 shrink-0 mb-2">
                                <span
                                    className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${isSaving ? 'text-indigo-500' : 'text-emerald-500 opacity-0'}`}
                                >
                        ...
                        <div className="bg-slate-50 dark:bg-white/5 p-6 rounded-3xl border border-slate-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
                                    {/* Image Previews */}
                                    {(formData.specs as any).gallery?.length > 0 ? (
                                        (formData.specs as any).gallery.slice(0, 4).map((img: string, i: number) => (
                                            <div
                                                key={i}
                                                className="w-12 h-12 shrink-0 rounded-lg bg-white dark:bg-black/20 border border-slate-200 dark:border-white/10 overflow-hidden"
                                            >
                                                <img
                                    ...
                                    {formData.specs.video_urls?.map((vid: string, i: number) => (
                                        <div
                                            key={`v-${i}`}
                                            className="w-12 h-12 shrink-0 rounded-lg bg-black relative overflow-hidden border border-slate-200 dark:border-white/10 group"
                                        >
                                            <img
                                    ...
                                    {formData.specs.pdf_urls?.map((pdf: string, i: number) => (
                                        <div
                                            key={`p-${i}`}
                                            className="w-12 h-12 shrink-0 rounded-lg bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 flex items-center justify-center"
                                            title="Document"
                                        >
                                    ...
                                    {!(formData.specs as any).gallery?.length &&
                                        !(formData.specs as any).video_urls?.length &&
                                        !(formData.specs as any).pdf_urls?.length && (
                    ...
                    <div className="flex justify-end pt-8 border-t border-slate-100 dark:border-white/5 mt-8">
                        <button
                            onClick={async () => {
                                const saved = await handleAutoSave(); // Force save before closing
