...
export default function VehicleCatalogPage() {
    ...
    const { tenantType, tenantId, userRole, tenantSlug } = useTenant();
    ...
    const fetchVehicles = async () => {
        ...
        try {
            ...
            (brands || []).forEach(brand => {
                ...
                mappedList.push({
                    id: brand.slug || brand.name, // Prefer slug for URL routing, fallback to name
                    brandId: brand.id,
                    brand: brand.name,
            ...
            <div className="space-y-10">
                ...
                <MasterListDetailLayout mode="list-only">
                    <ListPanel
                        ...
                        basePath={tenantSlug ? `/app/${tenantSlug}/dashboard/catalog/vehicles` : "/dashboard/catalog/vehicles"}
                        tight={true}
                        onQuickAction={(action, item) => {
                            if (action === 'edit') {
                                const base = tenantSlug ? `/app/${tenantSlug}/dashboard` : '/dashboard';
                                router.push(`${base}/catalog/vehicles/studio?modelId=${item.modelId}&brandId=${item.brandId}`);
                            }
                        }}
                    />
