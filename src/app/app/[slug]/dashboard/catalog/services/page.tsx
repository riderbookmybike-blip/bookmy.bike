...
export default function ServicesMasterPage() {
    ...
    const [dataSource, setDataSource] = React.useState<'LIVE' | 'MOCK'>('MOCK');
    ...
    const metrics = (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-8 mb-4">
            {[
                ...
                { label: 'Service Types', value: new Set(data.map(s => s.type)).size, icon: Layers, color: 'text-emerald-500' },
                { label: 'Avg Price', value: `₹ ${Math.round(data.reduce((acc, d) => acc + Number(d.value), 0) / (data.length || 1))}`, icon: IndianRupee, color: 'text-purple-500' },
