...
const quotesMock = [
    { id: 'Q101', customer: 'Rahul Sharma', model: 'Honda Activa 6G', price: '₹92,400', version: 'v2', status: 'SENT', date: '2 hours ago' },
    { id: 'Q102', customer: 'Amit Singh', model: 'TVS Jupiter 125', price: '₹1,05,200', version: 'v1', status: 'DRAFT', date: '5 hours ago' },
];
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-sm">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-white/5">
                        {leadsMock.map(lead => (
                            <div key={lead.id} className="grid grid-cols-12 gap-4 p-6 items-center hover:bg-slate-50 dark:hover:bg-white/2 transition-all group">
                                <div className="col-span-4 flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-black ${lead.type === 'MIGRATED' ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600'
                                        }`}>
                                        {lead.name[0]}
                                    </div>
                                    <div>
                                ...
                                <div className="col-span-2 text-center">
                                    <div className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tight">{lead.model}</div>
                                    <div className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Primary Interest</div>
                                </div>
                                <div className="col-span-2 text-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">Recent <span className="text-indigo-600">Quotations</span></h3>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-sm">
                        {quotesMock.map(quote => (
                            <div key={quote.id} className="p-8 border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/2 transition-all group">
                                <div className="flex items-start justify-between">
                                    <div className="flex gap-6">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3">
                                                <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">{quote.customer}</h4>
                                                <span className="bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded text-[10px] font-black text-slate-500 uppercase tracking-widest">{quote.id}</span>
                                                <span className="flex items-center gap-1 bg-indigo-500/10 text-indigo-500 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-indigo-500/20">
                                                    <History size={10} /> {quote.version}
                                                </span>
                                            </div>
                                            <p className="text-sm font-bold text-slate-500 mt-2">{quote.model}</p>
                                            <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-widest">Last updated {quote.date} • Sent via WhatsApp</p>
                                        </div>
                                    </div>
                                    <div className="text-right space-y-3">
                                        <div className="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter leading-none">{quote.price}</div>
                ...
                <div className="space-y-6">
                    <div className="bg-indigo-600 rounded-3xl p-8 text-white shadow-2xl shadow-indigo-600/20 relative overflow-hidden">
                        ...
                        <h3 className="text-lg font-black uppercase tracking-widest italic leading-tight">Instant <br />Configurator</h3>
                        <p className="text-xs text-indigo-100 font-bold mt-2 opacity-80 uppercase tracking-widest">Generate quote versions in seconds</p>

                        <div className="mt-8 space-y-4">
                            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                                <label className="text-[9px] font-black uppercase tracking-[0.2em] opacity-60">Selected Lead</label>
                                <div className="text-sm font-black mt-1">Suresh Raina (Migrated)</div>
                            </div>
                            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                                <label className="text-[9px] font-black uppercase tracking-[0.2em] opacity-60">Model Variant</label>
                    ...
                    <div className="bg-slate-900 dark:bg-white p-8 rounded-3xl text-white dark:text-slate-900 border border-white/10">
                        ...
                        <p className="text-2xl font-black mt-4 italic tracking-tighter leading-tight">4.2m Avg <br />Conversion Time</p>
        ...
        return (
            <div className="space-y-10">
                <div className="flex items-center justify-between mb-2">
                    <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">Global <span className="text-indigo-600">Fulfillment</span> Pipeline</h3>
                ...
                {bookingsMock.map(booking => (
                    <div key={booking.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-[32px] p-8 shadow-sm">
                        <div className="flex flex-col md:flex-row justify-between gap-8 items-start mb-10">
                            <div className="flex gap-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-3">
                                        <h4 className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">{booking.customer}</h4>
                                        <span className="bg-slate-100 dark:bg-white/10 px-3 py-1 rounded text-[10px] font-black text-slate-500 uppercase tracking-widest">{booking.id}</span>
                                    </div>
                                    <p className="text-sm font-bold text-slate-400 mt-2">{booking.model} • Red Metallic</p>
                                </div>
                            </div>
                            <div className="flex flex-col items-end gap-2">
                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expected Delivery</div>
                                <div className="text-lg font-black text-slate-900 dark:text-white italic tracking-tighter uppercase">24 Jan 2026</div>
                            </div>
                        </div>
                                ...
                                {Object.entries(booking.stages).map(([stage, status], idx) => (
                                    <div key={stage} className="flex flex-col items-center gap-4">
                                        ...
                                        <div className="text-center">
                                            <div className={`text-[10px] font-black uppercase tracking-widest ${status === 'ONGOING' ? 'text-indigo-600' : 'text-slate-400'
                                                }`}>
                                                {stage}
                                            </div>
                ...
                <div className="bg-slate-50 dark:bg-white/2 rounded-[32px] p-10 border border-slate-200 dark:border-white/10 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
                    <div>
                        <div className="text-3xl font-black text-slate-900 dark:text-white italic tracking-tighter">89%</div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">SLA Compliance</div>
                    </div>
                    <div className="border-x border-slate-200 dark:border-white/10">
                        ...
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Avg Final Delivery</div>
                    </div>
                    <div>
                        <div className="text-3xl font-black text-slate-900 dark:text-white italic tracking-tighter">98.2%</div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Stock Availability</div>
                    </div>
                </div>
            </div>
        );
