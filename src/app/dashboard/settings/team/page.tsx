                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100 dark:border-white/5">
                                <th className="p-6 text-xs font-black uppercase tracking-wider text-slate-400">Member</th>
                                ...
                                <tr key={member.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                    <td className="p-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-bold text-slate-900 dark:text-white">{member.profiles?.full_name || 'Unknown User'}</p>
                                                <p className="text-xs text-slate-500">{member.profiles?.email || member.profiles?.phone || 'No contact info'}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-6">
                                    ...
                                    <td className="p-6">
                                        <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${member.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400' :
                                            member.status === 'SUSPENDED' ? 'bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400' :
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-8 shadow-2xl space-y-6">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={() => setIsInviteOpen(false)} className="p-2 hover:bg-slate-100 rounded-full"><X size={20} /></button>
                        </div>
                        <form onSubmit={handleInvite} className="space-y-4">
                            <div>
                                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2 block">Email / Phone</label>
                                <input name="email" type="email" required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500" placeholder="colleague@example.com" />
                            </div>
                            <div>
                                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2 block">Role</label>
                                <select name="role" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500">
                            ...
                            <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-xl text-xs font-black uppercase tracking-wider">Send Invitation</button>
                        </form>
                    </div>
                </div>
            )}
