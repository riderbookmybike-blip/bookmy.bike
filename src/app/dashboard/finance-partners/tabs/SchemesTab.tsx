...
export default function SchemesTab({
    ...
    bankId: string;
    chargesMaster: any[];
    onSchemesUpdated?: (schemes: BankScheme[]) => void;
}) {
    ...
    const calculatedSchemes = useMemo(() => {
        const results = schemes.map(scheme => {
            ...
            // Add monthly fixed charges ON TOP of base EMI
            ...
            // Payout calculation
            const payoutAmount = scheme.payoutType === 'PERCENTAGE' ? (disbursal * scheme.payout) / 100 : scheme.payout;
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between mb-12 gap-8">
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mt-1 ml-12">
                        Live Scenario Analysis & Scheme Comparison
                    </p>
                </div>
                    ...
                    <div className="bg-slate-50 dark:bg-white/5 border border-dashed border-slate-300 dark:border-white/10 rounded-[32px] p-20 text-center flex flex-col items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1 uppercase font-bold tracking-widest">
                                Start by adding your first financial scheme
                            </p>
                        </div>
                        <button
                            onClick={() => {
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-12 flex-1">
                                        <div className="flex flex-col min-w-[200px]">
                                            <h4 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic">
                                                {scheme.name}
                                            </h4>
                                            <div className="flex items-center gap-3 mt-1">
                                                ...
                                                <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">
                                                    {scheme.interestRate}% {scheme.interestType}
                                                </span>
                                                <span className="text-slate-300 dark:text-slate-700">|</span>
                                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                                    {scheme.charges.length} Rules Applied
                                                </span>
                                        ...
                                        <div className="grid grid-cols-4 gap-4 flex-1">
                                            <div className="bg-slate-50 dark:bg-white/[0.03] backdrop-blur-md rounded-[20px] p-4 border border-slate-100 dark:border-white/[0.05] flex flex-col items-center">
                                                ...
                                                <div className="text-xl font-black text-slate-900 dark:text-white italic tracking-tighter">
                                                    ₹{scheme.simResults.emi.toLocaleString()}
                                                </div>
                                            </div>
                                            <div className="bg-slate-50 dark:bg-white/[0.03] backdrop-blur-md rounded-[20px] p-4 border border-slate-100 dark:border-white/[0.05] flex flex-col items-center">
                                                ...
                                                <div className="text-lg font-black text-slate-700 dark:text-slate-300 tracking-tighter">
                                                    ₹{scheme.simResults.downpayment.toLocaleString()}
                                                </div>
                                            </div>
                                            <div className="bg-slate-50 dark:bg-white/[0.03] backdrop-blur-md rounded-[20px] p-4 border border-slate-100 dark:border-white/[0.05] flex flex-col items-center">
                                                <div className="text-[7px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">
                                                ...
                                                <div className="text-lg font-black text-emerald-600 dark:text-emerald-400 tracking-tighter">
                                                    ₹{scheme.simResults.disbursal.toLocaleString()}
                                                </div>
                                            </div>
                                            <div className="bg-amber-50/50 dark:bg-amber-500/5 backdrop-blur-md rounded-[20px] p-4 border border-amber-100/50 dark:border-amber-500/10 flex flex-col items-center">
                                                ...
                                                <div className="text-lg font-black text-amber-600 dark:text-amber-500 tracking-tighter">
                                                    ₹{scheme.simResults.payout.toLocaleString()}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
