...
export default function BankList() {
    ...
    const finalData = banks.map(p => {
        const schemesCount = (p.config?.schemes?.length || 0);
        return {
            ...p,
    ...
    const metrics = (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
                ...
                { label: 'Active Schemes', value: finalData.reduce((acc, p) => acc + (p.rawSchemesCount || 0), 0), icon: Zap, color: 'text-emerald-500' },
        ...
        <div className="p-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-white/5 p-1 rounded-2xl w-fit mb-8 border border-slate-200 dark:border-white/5 mx-auto">
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'list'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'routing'
                        ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm shadow-black/5'
                        : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                >
