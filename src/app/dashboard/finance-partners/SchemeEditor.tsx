...
const EMPTY_SCHEME: BankScheme = {
    ...
    charges: [
        { id: generateDisplayId(), name: 'Processing Fee', type: 'PERCENTAGE', value: 2.0, calculationBasis: 'GROSS_LOAN_AMOUNT', impact: 'UPFRONT', taxStatus: 'NOT_APPLICABLE' }
    ],
    applicability: {
        brands: 'ALL',
        models: 'ALL',
...
export default function SchemeEditor({ initialScheme, onSave, onCancel, chargesMaster, bankId }: SchemeEditorProps) {
    ...
    useEffect(() => {
        ...
        const tenuresToCalculate = scheme.allowedTenures && scheme.allowedTenures.length > 0
        ...
        const emiResults = tenuresToCalculate.map(n => {
            ...
            scheme.charges.forEach(ch => {
                ...
                if (ch.type === 'FIXED') amount = ch.value;
                    ...
                    const rule = sortedRules.find(r => activeLoan >= (r.minAge || 0) && activeLoan <= (r.maxAge || 9999999) && r.tenure >= n);
            ...
            // Financial Metrics
            const totalPayout = scheme.payoutType === 'PERCENTAGE' ? (grossLoanAmount * scheme.payout / 100) : scheme.payout;
            const subventionVal = scheme.subvention || 0;
            const totalSubvention = scheme.subventionType === 'PERCENTAGE' ? (grossLoanAmount * subventionVal / 100) : subventionVal;
            ...
            <div className="flex-1 space-y-8 pb-20 px-1">
                ...
                <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[32px] p-8 mt-0 shadow-sm dark:shadow-2xl shadow-black/20">
                    <h3 className="text-slate-900 dark:text-white font-black text-xs uppercase tracking-widest mb-8 flex items-center gap-3">
                        <span className="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 text-xs flex items-center justify-center border border-blue-500/10 dark:border-blue-500/20 shadow-lg shadow-blue-500/5 dark:shadow-blue-500/10">1</span>
                        Scheme Basics
                    </h3>
                    <div className="grid grid-cols-2 gap-6 mb-4">
                        <div className="col-span-2">
                            <div className="flex items-center justify-between mb-2">
                                <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest block">Scheme Name</label>
                                <div className="flex items-center gap-2">
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Scheme CID:</span>
                    ...
                    <div className="flex items-center justify-between p-4 bg-blue-50/50 dark:bg-blue-500/5 border border-blue-500/20 dark:border-blue-500/10 rounded-2xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h4 className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Platform Primary</h4>
                                <p className="text-[9px] text-slate-500 uppercase font-bold tracking-tight">Main fallback scheme for marketplace</p>
                            </div>
                        </div>
                        <button
                            type="button"
                        ...
                        >
                            <div className={`w-6 h-6 rounded-full bg-white shadow-sm transition-all transform ${scheme.isPrimary ? 'translate-x-6' : 'translate-x-0'}`} />
                        </button>
                    </div>
                </section>
                ...
                <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[32px] p-8 mt-8 shadow-sm dark:shadow-2xl shadow-black/20">
                    <h3 className="text-slate-900 dark:text-white font-black text-xs uppercase tracking-widest mb-8 flex items-center gap-3">
                        <span className="w-8 h-8 rounded-xl bg-orange-50 dark:bg-orange-600/20 text-orange-600 dark:text-orange-400 text-xs flex items-center justify-center border border-orange-500/10 dark:border-orange-500/20 shadow-lg shadow-orange-500/5 dark:shadow-orange-500/10">1.5</span>
                        Scheme Applicability
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {/* Brands */}
                        <div className="space-y-4">
                            <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest block">Brand Scope</label>
                            <div className="flex bg-slate-50 dark:bg-black/40 p-1 rounded-xl border border-slate-200 dark:border-white/10">
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), brands: 'ALL' } })}
                                ...
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), brands: [] } })}
                                    className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${scheme.applicability?.brands !== 'ALL' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm' : 'text-slate-400'}`}
                                >
                                    Specific
                                </button>
                            ...
                            {scheme.applicability?.brands !== 'ALL' && (
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                                ...
                                                if (val && Array.isArray(scheme.applicability?.brands) && !scheme.applicability?.brands.includes(val)) {
                                                    setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), brands: [...(scheme.applicability?.brands as string[] || []), val] } });
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {Array.isArray(scheme.applicability?.brands) && scheme.applicability?.brands.map(b => (
                                            <span key={b} className="bg-blue-500/10 text-blue-500 text-[8px] font-black px-2 py-0.5 rounded border border-blue-500/20 flex items-center gap-1">
                                                {b} <button onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), brands: (scheme.applicability?.brands as string[]).filter(x => x !== b) } })}><Plus size={8} className="rotate-45" /></button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {/* Models */}
                        <div className="space-y-4">
                            <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest block">Model Scope</label>
                            <div className="flex bg-slate-50 dark:bg-black/40 p-1 rounded-xl border border-slate-200 dark:border-white/10">
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), models: 'ALL' } })}
                                ...
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), models: [] } })}
                                    className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${scheme.applicability?.models !== 'ALL' ? 'bg-white dark:bg-slate-800 text-emerald-600 shadow-sm' : 'text-slate-400'}`}
                                >
                                    Specific
                                </button>
                                ...
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                                const val = (e.target as HTMLInputElement).value.trim();
                                                if (val && Array.isArray(scheme.applicability?.models) && !scheme.applicability?.models.includes(val)) {
                                                    setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), models: [...(scheme.applicability?.models as string[] || []), val] } });
                                                    (e.target as HTMLInputElement).value = '';
                                                }
                                            }
                                        }}
                                    ...
                                    />
                                    <div className="flex flex-wrap gap-1.5">
                                        {Array.isArray(scheme.applicability?.models) && scheme.applicability?.models.map(m => (
                                            <span key={m} className="bg-emerald-500/10 text-emerald-500 text-[8px] font-black px-2 py-0.5 rounded border border-emerald-500/20 flex items-center gap-1">
                                                {m} <button onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), models: (scheme.applicability?.models as string[]).filter(x => x !== m) } })}><Plus size={8} className="rotate-45" /></button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {/* Dealerships */}
                        <div className="space-y-4">
                            <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest block">Dealer Scope</label>
                            <div className="flex bg-slate-50 dark:bg-black/40 p-1 rounded-xl border border-slate-200 dark:border-white/10">
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), dealerships: 'ALL' } })}
                                ...
                                <button
                                    onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), dealerships: [] } })}
                                    className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${scheme.applicability?.dealerships !== 'ALL' ? 'bg-white dark:bg-slate-800 text-purple-600 shadow-sm' : 'text-slate-400'}`}
                                >
                                    Specific
                                </button>
                                ...
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                                const val = (e.target as HTMLInputElement).value.trim();
                                                if (val && Array.isArray(scheme.applicability?.dealerships) && !scheme.applicability?.dealerships.includes(val)) {
                                                    setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), dealerships: [...(scheme.applicability?.dealerships as string[] || []), val] } });
                                                    (e.target as HTMLInputElement).value = '';
                                                }
                                            }
                                        }}
                                    ...
                                    />
                                    <div className="flex flex-wrap gap-1.5">
                                        {Array.isArray(scheme.applicability?.dealerships) && scheme.applicability?.dealerships.map(d => (
                                            <span key={d} className="bg-purple-500/10 text-purple-500 text-[8px] font-black px-2 py-0.5 rounded border border-purple-500/20 flex items-center gap-1">
                                                {d} <button onClick={() => setScheme({ ...scheme, applicability: { ...(scheme.applicability || {}), dealerships: (scheme.applicability?.dealerships as string[]).filter(x => x !== d) } })}><Plus size={8} className="rotate-45" /></button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                ...
                <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[32px] p-8 mt-8 shadow-sm dark:shadow-2xl shadow-black/20">
                    <h3 className="text-slate-900 dark:text-white font-black text-xs uppercase tracking-widest mb-8 flex items-center gap-3">
                        <span className="w-8 h-8 rounded-xl bg-emerald-50 dark:bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 text-xs flex items-center justify-center border border-emerald-500/10 dark:border-emerald-500/20 shadow-lg shadow-emerald-500/5 dark:shadow-emerald-500/10">2</span>
                        Lending Criteria
                    </h3>
                    <div className="grid grid-cols-3 gap-6">
                        ...
                        <div>
                            <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-2 block">Max Loan Amount</label>
                            <div className="relative">
                        ...
                        <div className="col-span-3">
                            ...
                            <div className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-[14px] p-2 min-h-[48px] flex flex-wrap gap-2 items-center focus-within:border-blue-500/50 transition-all">
                                {scheme.allowedTenures?.sort((a, b) => a - b).map(t => (
                                    <span key={t} className="bg-blue-500/10 text-blue-500 border border-blue-500/20 px-3 py-1 rounded-full text-xs font-mono font-bold flex items-center gap-2 group">
                                        {t}
                                        <button
                                            onClick={() => {
                                                const newTenures = scheme.allowedTenures?.filter(x => x !== t) || [];
                                ...
                                <input
                                    ...
                                    onKeyDown={e => {
                                        if (e.key === 'Enter' || e.key === ' ' || e.key === ',') {
                                            e.preventDefault();
                                            const parts = tenureInputText.split(/[\s,]+/).map(s => Number(s.trim())).filter(n => !isNaN(n) && n > 0);
                                            if (parts.length > 0) {
                                                const current = scheme.allowedTenures || [];
                                                const combined = Array.from(new Set([...current, ...parts])).sort((a, b) => a - b);
                                                const updates: Partial<BankScheme> = {
                                        ...
                                        if (e.key === 'Backspace' && tenureInputText === '' && scheme.allowedTenures?.length) {
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-950/20 dark:to-slate-900/50 rounded-2xl p-6 border border-emerald-200 dark:border-emerald-500/10">
                                <label className="text-[10px] uppercase font-black text-emerald-600 dark:text-emerald-400 tracking-widest mb-4 flex items-center gap-2 block">
                                    <Tag size={12} /> EMI Waiver
                                    <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 text-[8px]">Benefit</span>
                                </label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        ...
                                        onChange={e => setScheme({ ...scheme, emiWaiverCount: Number(e.target.value) || 0 })}
                                        className="w-full bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-500/20 rounded-xl px-4 py-3 text-emerald-700 dark:text-emerald-300 font-bold text-lg outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20 transition-all"
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-black text-emerald-500/40 uppercase tracking-widest">Waived</span>
                                </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-red-50 to-white dark:from-red-950/20 dark:to-slate-900/50 rounded-2xl p-6 border border-red-200 dark:border-red-500/10">
                                <label className="text-[10px] uppercase font-black text-red-600 dark:text-red-400 tracking-widest mb-4 flex items-center gap-2 block">
                                    <Minus size={12} /> Subvention
                                    <span className="px-2 py-0.5 rounded bg-red-500/10 text-red-600 text-[8px]">Cost</span>
                                </label>
                                <div className="grid grid-cols-12 gap-2">
                                    <div className="col-span-8">
                                        <input
                                            ...
                                            onChange={e => setScheme({ ...scheme, subvention: Number(e.target.value) || 0 })}
                                    ...
                                    <div className="col-span-4">
                                        <select
                                            ...
                                            onChange={e => setScheme({ ...scheme, subventionType: e.target.value as any })}
                                            className="w-full bg-white dark:bg-slate-800 border border-red-200 dark:border-red-500/20 rounded-xl px-3 py-3 font-black text-sm text-red-600 dark:text-red-300 outline-none appearance-none focus:border-red-500/50 focus:ring-2 focus:ring-red-500/20 transition-all"
                                        >
                                            <option value="PERCENTAGE">% Perc.</option>
                                            <option value="FIXED">₹ Fixed</option>
                                        </select>
                        ...
                        <div className="mb-8 p-4 bg-slate-50 dark:bg-white/[0.02] rounded-[24px] border border-slate-200 dark:border-white/5">
                            <div className="flex items-center justify-between mb-4 px-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Master Templates</span>
                                </div>
                                <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{chargesMaster.length} Available</span>
                            </div>
                                ...
                                {chargesMaster.map(master => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`text-[9px] font-black uppercase tracking-tight px-4 py-2 rounded-xl border transition-all flex items-center gap-2 ${isAlreadyAdded
                                        ...
                                        >
                                            {isAlreadyAdded ? <Settings size={12} className="opacity-40" /> : <Plus size={12} />}
                                            {master.name}
                                            {!isAlreadyAdded && <span className="ml-1 opacity-40 font-bold">({master.value}{master.type === 'PERCENTAGE' ? '%' : '₹'})</span>}
                                        </button>
                                    );
                                })}
                            </div>
                                                        ...
                                                        <div className="flex flex-col gap-3 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-200 dark:border-white/10 shadow-xl dark:shadow-2xl animate-in fade-in slide-in-from-right-2 duration-300 w-64">
                                                            <div className="flex items-center gap-2">
                                                                <input
                                                                    ...
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter') {
                                                                            const tenures = newTenureValue.split(',').map(s => Number(s.trim())).filter(t => !isNaN(t) && t > 0);
                                                                            if (tenures.length > 0) {
                                                                                ...
                                                                                let allNewRules = [...(charge.tableData || [])];
                                                                                tenures.forEach(t => {
                                                                                    const exists = charge.tableData?.some(r => r.tenure === t);
                                                                                    if (!exists) {
                                                                                        ...
                                                                                        allNewRules = [...allNewRules, ...newRules];
                                                                                    }
                                                                                });
                                                                                updateCharge(charge.id, { tableData: allNewRules });
                                                                                setNewTenureValue('');
                                                            ...
                                                            <div className="space-y-2">
                                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Quick Presets</p>
                                                            ...
                                                            <div className="pt-2 border-t border-slate-100 dark:border-white/5 flex justify-between items-center">
                                                                ...
                                                                <p className="text-[8px] text-slate-400 italic">Comma separated for bulk</p>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <button
                                                ...
                                                <table className="w-full border-collapse border border-white/5">
                                                    <thead>
                                                        <tr className="bg-slate-50 dark:bg-white/5">
                                                            <th className="p-2 border border-slate-200 dark:border-white/5 text-[9px] uppercase font-black text-slate-400 dark:text-slate-500 w-24">Min Amount</th>
                                                            <th className="p-2 border border-slate-200 dark:border-white/5 text-[9px] uppercase font-black text-slate-400 dark:text-slate-500 w-24">Max Amount</th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        {(!charge.tableData || charge.tableData.length === 0) && (
                                                            <tr>
                                                                <td colSpan={10} className="text-center py-10 bg-slate-50 dark:bg-black/10">
                                                                    <p className="text-[10px] text-slate-600 uppercase font-black tracking-widest">Spreadsheet Empty</p>
                                                                    <p className="text-[9px] text-slate-400 dark:text-slate-700 mt-1">Click "+ Add Range" or "+ Add Tenure" to start.</p>
                                                                </td>
                                                            </tr>
                                                        )}
                                                    </tbody>
