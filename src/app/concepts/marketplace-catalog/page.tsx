    ...
    return (
        ...
        >
            ...
            <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6">
                ...
                <div className="flex items-center gap-3">
                    <button className="rounded-full border border-slate-300 dark:border-white/20 px-5 py-2 text-xs font-black uppercase tracking-[0.25em] text-slate-700 dark:text-slate-200">Filters</button>
            ...
            <main className="mx-auto w-full max-w-6xl px-6 pb-16">
                <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-6 py-4">
                    <div>
                        ...
                        <h1 className="text-3xl font-black uppercase text-slate-900 dark:text-white">72 matches</h1>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
                        ...
                        <button className="rounded-full border border-slate-200 dark:border-white/10 px-4 py-2">Price</button>
                        <button className="rounded-full border border-slate-200 dark:border-white/10 px-4 py-2">EMI</button>
                        <button className="rounded-full border border-slate-200 dark:border-white/10 px-4 py-2">Range</button>
                    </div>
                </div>

                <div className="mt-8 grid gap-6 lg:grid-cols-[280px_1fr]">
                    <aside className="space-y-6 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-6 shadow-sm">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Search</p>
                            <input
                                placeholder="Find model"
                                className="mt-3 w-full rounded-2xl border border-slate-200 dark:border-white/10 px-4 py-3 text-sm font-medium bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                            />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {['Scooter', 'Bike', 'EV', 'Commuter'].map((segment) => (
                                <label key={segment} className="flex items-center gap-3 text-sm font-semibold text-slate-600 dark:text-slate-400">
                                    <input type="checkbox" className="h-4 w-4 rounded border-slate-300" />
                                    {segment}
                                </label>
                            ))}
                        </div>
                        <button className="w-full rounded-full border border-slate-300 dark:border-white/20 px-4 py-3 text-xs font-black uppercase tracking-[0.3em] text-slate-700 dark:text-slate-200">
                            Clear filters
                        </button>
                    </aside>
                    ...
                    <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                        {['Jupiter SmartX', 'Activa 125', 'Access 125', 'Ntorq 150', 'Chetak EV', 'Avenis'].map((item) => (
                            <div key={item} className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-5 shadow-sm">
                                <div className="h-36 rounded-2xl bg-slate-100" />
                                <div className="mt-4 flex items-start justify-between">
                                    <div>
                                        <h3 className="text-lg font-black uppercase text-slate-900 dark:text-white">{item}</h3>
